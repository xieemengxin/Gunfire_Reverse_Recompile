# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/passive/p5317.pyc
# RelativePath: clientlogic/cl_platformdata/pc/passive/p5317.pyc
...
def ForceFullCharge(oWarrior, oLifeCycle, iPerform = 9020, iDefaultMaxCount = 10):
    ...
    if not oPerform:
        return None
    sKey = oLifeCycle.Key()
    if 'CommonMaxCount' not in oPerform.m_Attr or 'TriggerTimes' not in oPerform.m_Attr:
        CustomAction5317InitAttr(oWarrior, oLifeCycle, {
            'MaxCount': iDefaultMaxCount,
            'Perform': iPerform })
    iMaxCount = oPerform.CalAttr('CommonMaxCount') if 'CommonMaxCount' in oPerform.m_Attr else iDefaultMaxCount
    oPerform.AttrChange('TriggerTimes', sKey, 0, iMaxCount)
...
def Action1(oWarrior, oLifeCycle):
    ...
    if cl_customconfig.IsInfiniteChargeEnabled():
        ForceFullCharge(oWarrior, oLifeCycle, 9020, 10)
...
'sKey': 'StoreDam' }) + Func717(*a, **{
'sArg': 'AddDam' })))
        if not cl_customconfig.IsInfiniteChargeEnabled():
            CustomAction5317ChangeCount(oWarrior, oEventCB.GetCBLifeCycle(), {
                'Perform': 9020 })
    if cl_customconfig.IsInfiniteChargeEnabled():
        ForceFullCharge(oWarrior, oEventCB.GetCBLifeCycle(), 9020, 10)
