...
class CWarData(object):
    ...
    def GetWeaponUpgradeCost(self, iCurGrade):
        ...
        iCost = dUpgrade[iNewGrade]
        return cl_customconfig.ScaleInt(iCost, cl_customconfig.GetFloat('economy.weapon_upgrade_cost_multiplier', 1.0))
    ...
    def GetMaxInscriptionNum(self):
        dInscriptionNum = self.m_WeaponGradeInfo.get('InscriptionNum', { })
        if dInscriptionNum:
            return max(6, max(dInscriptionNum.values()))
        return 6
