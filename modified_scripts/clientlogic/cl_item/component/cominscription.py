...
# 铭刻内部总槽位提升到 10，客户端展示仍限制为 6
# 客户端显示顺序：双子优先，其次按铭刻品质从高到低
# 可按战场/层关配置自定义掉落铭刻数量，最终取原始值和自定义值中的较大者
# 移除所有铭刻互斥关系与武器适配限制，所有铭刻都可在所有武器上生成
# ResetInscription / RecastGemini / 封印铭刻池 都不再按 ExcludeList 或 CheckValidItem 做过滤
# 战场侧 GetMaxInscriptionNum() 返回内部上限 10；对客户端的返回单独裁到 6
...
import cl_formula
import cl_msgcenter
MAX_INSCRIPTION_NUM = 10
CLIENT_INSCRIPTION_NUM = 6
BASE_GEMINI_NUM = 1
MAX_GEMINI_NUM = 1
LEAST_GEMINI_NUM = 3
...
# ======================== 铭文数量配置表 (按关卡进度) ========================
INSCRIPTION_NUM_BY_LEVEL = {
    3001: {(1, 0): 3, (1, 1): 10, (1, 4): 10, (2, 1): 10, (3, 1): 10},
    3002: {},
    3003: {},
    3004: {},
    3005: {(1, 0): 3, (1, 1): 10, (1, 2): 10, (1, 3): 10, (3, 1): 10},
    3006: {(1, 0): 3},
    3007: {(1, 0): 3, (1, 1): 10, (1, 4): 10, (2, 1): 10, (3, 1): 10},
    3008: {(1, 0): 10, (2, 1): 10, (2, 4): 10},
    3009: {(1, 0): 10, (2, 1): 10, (2, 4): 10},
    3010: {(1, 0): 3, (1, 2): 10, (1, 3): 10, (1, 4): 10, (3, 1): 10},
    3011: {(1, 0): 3},
    3012: {(1, 0): 3, (1, 1): 10, (1, 3): 10, (2, 1): 10, (3, 1): 10},
    3020: {(1, 0): 3, (1, 1): 10, (1, 4): 10, (2, 1): 10, (3, 1): 10},
    3030: {(1, 0): 3, (1, 1): 10, (1, 4): 10, (2, 1): 10, (3, 1): 10},
}
def GetInscriptionNumByLevel(iWarType, iLayer, iLevel):
    """根据模式ID和当前层/关查配置表, 返回铭文数量"""
    dConfig = INSCRIPTION_NUM_BY_LEVEL.get(iWarType, {})
    iNum = 0
    for (layer, level), num in sorted(dConfig.items(), key = lambda item: item[0]):
        if layer > iLayer:
            continue
...
g_ClientInscriptionOrder = {
    INSCRIPTION_TYPE_GEMINI: 0,
    INSCRIPTION_TYPE_EXCLUSIVE: 1,
    INSCRIPTION_TYPE_RARE: 2,
    INSCRIPTION_TYPE_NORMAL: 3 }
g_ActionType2UpType = {
...
class CInscriptionComponent(CItemComponent):
    ...
    def InitInscriptionNum(self):
        ...
        if not iNum:
            ...
            # 用本地配置表提升铭文数量 (取两者中较大值)
            iWarNo = getattr(oGame.m_WarData, 'm_WarNo', 0)
            iCustom = GetInscriptionNumByLevel(iWarNo, iLayer, iLevel)
            if iCustom > iNum:
                iNum = iCustom
    ...
    def GetAllInscription(self, iFill = 1, lstDisable = None, iIgnoreDisable = 1):
        ...
        if iFill:
            lstPerform = self.GetClientInscription(lstPerform)
        return lstPerform

    
    def GetClientInscription(self, lstPerform = None, iFill = 1):
        if lstPerform is None:
            lstPerform = self.m_Inscription[:]
        lstShow = []
        for idx, iPerform in enumerate(lstPerform):
            if not iPerform:
                continue
            ...
            iPriority = g_ClientInscriptionOrder.get(iType, 99)
            lstShow.append((iPriority, idx, iPerform))
        
        lstShow.sort(key = lambda info: (info[0], info[1]))
        lstResult = [iPerform for (_, _, iPerform) in lstShow[:CLIENT_INSCRIPTION_NUM]]
        if iFill and len(lstResult) < CLIENT_INSCRIPTION_NUM:
            lstResult.extend([
                0] * (CLIENT_INSCRIPTION_NUM - len(lstResult)))
        return lstResult
    ...
    def RecastGemini(self):
        ...
        if iNum > iCur:
            ...
            if iChooseNum > MAX_GEMINI_NUM:
                iChooseNum = MAX_GEMINI_NUM
                oItem = self.m_Item
                oGame = oItem.m_Game
                if oItem and oGame:
    ...
    def ResetInscription(self):
        ...
        if lstGemini:
            iGeminiNum = len(lstGemini)
            (iAdd, _) = self.ChooseInscriptionByType(INSCRIPTION_TYPE_GEMINI, iGeminiNum, lstGemini)
            if iAdd < iGeminiNum:
                for iPerform in lstGemini[iAdd:iGeminiNum]:
                    self.AppendInscription(iPerform)
            iNum -= iGeminiNum
    ...
    def GetRestTypeNum(self, iType):
        ...
        if iType == INSCRIPTION_TYPE_GEMINI:
            ...
            if iTempGeminiInscription:
                iMax = max(iMax, iTempGeminiInscription)
    ...
    def GetProp(self, iType, iGrade):
        if iType == INSCRIPTION_TYPE_GEMINI:
            if self.m_InscriptionNum < LEAST_GEMINI_NUM or self.GetRestTypeNum(iType) < 1:
                return 0
            return 10000
        oGame = self.m_Item.m_Game
    ...
    def OnItemRemoveFromContainer(self, oItem, oOwner):
        ...
        if not oOwner or not hasattr(oOwner, 'm_WieldCon'):
            return None
        for oWeapon in oOwner.m_WieldCon.GetAllItemByType(itemdef.EQUIP_TYPE_MAINWEAPON):
            if not oWeapon or oWeapon.m_ID == oTarget.m_ID:
                continue
            oOtherInscriptionCom = oWeapon.GetComponent('Inscription')
            if oOtherInscriptionCom:
                oOtherInscriptionCom.RefreshGeminiState()
    ...
    def CheckCanEnableGemini(self, iSID):
        oPerformCom = self.m_Item.GetComponent('Perform')
        if not oPerformCom:
            return 0
        oPerform = oPerformCom.GetPerform(iSID)
        if oPerform and oPerform.m_Enable:
            return 0
        return 1
    ...
    def TryEnableGemini(self, iSID):
        ...
        if not oOtherInscriptionCom:
            return None
        if not self.CheckCanEnableGemini(iSID):
            return None
        oOwner = self.m_Item.GetOwner()
        if oOwner:
    ...
    def DisableGeminiEffect(self, iSID):
        oPerformCom = self.m_Item.GetComponent('Perform')
        if not oPerformCom:
            return None
        oPerform = oPerformCom.GetPerform(iSID)
    ...
    def RefreshGeminiState(self):
        lstGemini = self.GetInscriptionByType(INSCRIPTION_TYPE_GEMINI)
        if not lstGemini:
            return None
        oOtherInscriptionCom = self.GetAnotherWeaponInscriptionCom()
        if not oOtherInscriptionCom:
            for iSID in lstGemini:
                self.DisableGeminiEffect(iSID)
            return None
        for iSID in lstGemini:
            self.TryEnableGemini(iSID)
    ...
    def RefreshDisbleInscription(self):
        ...
        lstDisable = self.GetDisableInscription()
        lstClientDisable = []
        for iInscription in self.m_Inscription:
            if iInscription in lstDisable:
                lstClientDisable.append(iInscription)
        
        oItem.GS2CItemPropChange('DisableInscription', self.GetClientInscription(lstClientDisable, iFill = 0))
