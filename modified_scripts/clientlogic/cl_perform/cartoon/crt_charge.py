...
class ChargeCartoon(CBaseCartoon):
    ...
    def OnArrive(cls, oSkill, dCartoon):
        ...
        if pfobj.m_PFType == PF_TYPE_CHARGE:
            iChargeLevel = dCartoon.get('ChargeLevel', dCartoon.get('MaxChargeLevel', 0))
            if dCartoon.get('PerBulletCost', 0):
                iPreBulletCostNum = iChargeLevel * dCartoon['PerBulletCost']
                oSkill.m_Collect['BaseBullet'] = iPreBulletCostNum
                if dCartoon.get('AllowMaxChargeLowAmmo'):
                    pfobj = oAttack.GetPerform(oSkill.m_Base['pfid'], oSkill.m_Base['Weapon'])
                    if pfobj:
        ...
        if dCartoon.get('ChargeLevel', 0) > 0:
            cls.Trigger(oSkill)
    ...
    def HitTargetServer(cls, oSkill, dCartoon):
        if dCartoon['StartFrame'] + dCartoon['WaitFrame'] <= oSkill.m_Game.GetFrameNum():
            dCartoon['Over'] = 1
            if cl_customconfig.IsInfiniteChargeEnabled():
                dCartoon['ChargeLevel'] = dCartoon.get('MaxChargeLevel', dCartoon.get('ChargeLevel', 0))
...
def GetMaxChargeLevel(oSkill, dCartoon):
    if cl_customconfig.IsInfiniteChargeEnabled():
        return dCartoon.get('MaxChargeLevel', 0)
