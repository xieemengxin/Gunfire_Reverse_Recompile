...
def PassiveSetLinkAttr(oTarget, oLifeCycle, sAttr, fMaxValue = 0):
    ...
    def GetOtherWeapon(oTarget, oWeapon):
    ...
    def ClearLinkAttr(oTarget, oLifeCycle):
        ...
        if not oLinkAttr or getattr(oLinkAttr, 'm_Type', '') != 'Link':
            return None
        oSelfAttr = oLinkAttr.DelLink(oWeapon.m_ID)
        if oSelfAttr:
            oWeapon.DirectSetAttr(sAttr, oSelfAttr)
        oOtherWeapon = GetOtherWeapon(oTarget, oWeapon)
        if not oOtherWeapon:
            return None
    ...
    if not oSelfAttr or not oConnectAttr:
        return None
    if getattr(oSelfAttr, 'm_Type', '') == 'Link' or getattr(oConnectAttr, 'm_Type', '') == 'Link':
        return None
...
def PassiveSetLinkPFBulletAttr(oTarget, oLifeCycle, iLinkMul):
    ...
    def ClearLinkAttr(dLinkPerform, oTarget, oLifeCycle):
        for iWeapon, iPerform in dLinkPerform.items():
            ...
            if not oLinkAttr or oLinkAttr.m_Type != 'Link':
                continue
