...
def CheckExtraInscription(oNpc, oHero, oItem):
    ...
    if not oInscriptionCom:
        return OPTION_STATUS_NOINTERACT
    if (oHero.Query('SuitExtraInscription', 0) or oHero.Query('ExtraInscription', 0)) and not oItem.CheckHasMaxInscriptionNum():
        return OPTION_STATUS_SHOW
    return OPTION_STATUS_HIDE
...
class CSmithNpc(mobject.CNPC):
    ...
    def GetRecastCost(self, oWeapon):
        ...
        return cl_customconfig.ScaleInt(iCost, cl_customconfig.GetFloat('economy.weapon_recast_cost_multiplier', 1.0))
    ...
    def GetExtraUpgradeCost(self, oWeapon):
        ...
        if self.m_ID in dCost:
            return cl_customconfig.ScaleInt(dCost[self.m_ID], cl_customconfig.GetFloat('economy.weapon_extra_upgrade_cost_multiplier', 1.0))
        return 0
