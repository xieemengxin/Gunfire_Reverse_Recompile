# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_minigame/mg_equip.pyc
# RelativePath: clientlogic/cl_minigame/mg_equip.pyc
# Source Generated with Decompyle++
# File: mg_equip.pyc (Python 3.6)

# 初始武器魔盒强制提供 3 把候选武器
# 若按初始弹药类型过滤后不足 3 把，则回退到全部已解锁主武器池补满
...
def FilterWeaponWeight(dWeight):
    return cl_customconfig.GetFilteredWeight(dWeight, cl_customconfig.GetWeaponWhitelist(), cl_customconfig.GetWeaponBlacklist())
...
class CDropEquipGameData(CBaseGameData):
    ...
    def GetChooseWeight(cls, oHero):
        if cls.m_ChooseWeight:
            return FilterWeaponWeight(dict(cls.m_ChooseWeight))
        dWeight = dict.fromkeys(oHero.Query('Illus')['Weapon'], 1)
        for iSID in cls.m_FilterList:
            if iSID in dWeight:
                dWeight.pop(iSID)
        
        return FilterWeaponWeight(dWeight)
...
class CInitEquipDropChoose(CMultipleEquipDropChoose):
    ...
    def GetPlayerInitDropChoose(cls, oPlayer):
        ...
        for iWeapon in lstUnlockWeapon:
            ...
            if not clsWeapon:
                continue
            if clsWeapon.m_Type & itemdef.EQUIP_TYPE_MAINWEAPON != itemdef.EQUIP_TYPE_MAINWEAPON:
                continue
            dAllMainWeapon[iWeapon] = 1
            iBulletType = clsWeapon.GetBulletType()
            if iBulletType in dInitWeaponDrop and dInitWeaponDrop[iBulletType]:
                dChoose[iWeapon] = 1

        if len(dChoose) < cls.m_MaxChooseLen:
            return dAllMainWeapon
        return dChoose
...
class CRewardChooseInitWeaponGame(CRewardChooseEquipGame):
    ...
    def GetRewardInfo(self):
        ...
        lstEquip = []
        setChosen = set()
        while len(lstEquip) < iChooseCnt:
            lstNew = oChoosePool.QueryChoose(self.m_SID)
            if not lstNew:
                break
            for iEquip in lstNew:
                if iEquip in setChosen:
                    continue
                setChosen.add(iEquip)
                lstEquip.append(iEquip)
                if len(lstEquip) >= iChooseCnt:
                    break
        if lstEquip:
            lstUse = lstEquip[:iChooseCnt]
            for idx, iEquip in enumerate(lstUse):
                oEquip = cl_item.CreateEquip(oGame, iEquip, iGrade, oOwner = oGame.GetObject(self.m_Player), iSource = ITEM_SOURCE_CHOOSE)
                if oEquip.Type() & EQUIP_TYPE_MAINWEAPON:
                    oBulletcom = oEquip.GetComponent('Bullet')
