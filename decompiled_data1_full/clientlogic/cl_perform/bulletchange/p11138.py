# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_perform/bulletchange/p11138.pyc
# RelativePath: clientlogic/cl_perform/bulletchange/p11138.pyc
# Source Generated with Decompyle++
# File: p11138.pyc (Python 3.6)
...
def DoCallBackAction0(oWarrior, pfBulletChange, oSkill, dClientInfo):
    if condition.CheckSkillMainPF(oWarrior, pfBulletChange) and condition.CheckRandom(oWarrior, pfBulletChange, oSkill, dClientInfo, 100, 40):
...
def DoCallBackAction1(oWarrior, pfBulletChange, oSkill, dClientInfo):
    if condition.CheckSkillMainPF(oWarrior, pfBulletChange):
        if condition.CheckRandom(oWarrior, pfBulletChange, oSkill, dClientInfo, 100, 40):
            action.BulletChangeExtBulletUse(oWarrior, pfBulletChange, oSkill, dClientInfo, 1)
        if condition.CheckRandom(oWarrior, pfBulletChange, oSkill, dClientInfo, 100, 40):
            action.BulletChangeExtBulletUse(oWarrior, pfBulletChange, oSkill, dClientInfo, 1)
