# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_perform/bulletchange/p11122.pyc
# RelativePath: clientlogic/cl_perform/bulletchange/p11122.pyc
# Source Generated with Decompyle++
# File: p11122.pyc (Python 3.6)
...
def DoCallBackAction0(oWarrior, pfBulletChange, oSkill, dClientInfo):
    if condition.CheckFromPointPerform(oWarrior, pfBulletChange, dClientInfo, 9312):
        if condition.CheckOwnServerState(oWarrior, pfBulletChange, oSkill, dClientInfo, 1763) and condition.CheckRandom(oWarrior, pfBulletChange, oSkill, dClientInfo, 10000, 2000):
            action.BulletChangeCBRevertPFBulletByFightType(oWarrior, pfBulletChange, oSkill, dClientInfo, 2)
        else:
            action.BulletChangeCBRevertPFBulletByFightType(oWarrior, pfBulletChange, oSkill, dClientInfo, 1)
    else:
        action.BulletChangeCBCostPFBullet(oWarrior, pfBulletChange, oSkill, dClientInfo, 300)
