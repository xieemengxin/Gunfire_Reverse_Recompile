# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/petpf/p7311.pyc
# RelativePath: clientlogic/cl_platformdata/pc/petpf/p7311.pyc
# Source Generated with Decompyle++
# File: p7311.pyc (Python 3.6)
...
class CCartoon0(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 120 / cl_action.GetSkillVarCache(skill, 'AttackSpeed')), 1)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgSelfPos(skill), cl_action.CrtArgSkillEndPos(skill), [
                8,
                3,
                110], attshape = ATT_SHAPE_SECTOR, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE, multipleExplode = False, explosionDelay = 0)
...
def Action(skill):
    cl_action.SetSkillVarCache(skill, 'AttackSpeed', cl_action.Clamp(skill, 160 / (skill.m_Cache['AttSpeed'] if skill.m_Cache['AttSpeed'] > 0 else 160), 0.4, 2.5))
