# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/petpf/p7336.pyc
# RelativePath: clientlogic/cl_platformdata/pc/petpf/p7336.pyc
# Source Generated with Decompyle++
# File: p7336.pyc (Python 3.6)
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, OBJ_ENEMY, SKILLCACHE_INT, SKILLCACHE_LSTINT, SKILLCACHE_RANDOM, WARRIOR_SUMMON
...
class CCartoon6(EntityCurveCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (0, 0, 0), cl_action.CrtArgTargetPos(skill, notContainDying = False), 0.3, 1067 if cl_action.CheckSkillRandomInRange(skill, 0, 33) else 1068 if cl_action.CheckSkillRandomInRange(skill, 34, 67) else 1069, 180, 35, 60, 0.5, targettype = OBJ_ENEMY, summonID = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[0], forceDel = True)
...
def Action(skill):
    if cl_action.CheckSkillRandomInRange(skill, 0, 33):
        cl_action.ModifySkillCache(skill, 'ElementType', 1024)
    elif cl_action.CheckSkillRandomInRange(skill, 34, 67):
        cl_action.ModifySkillCache(skill, 'ElementType', 512)
    else:
        cl_action.ModifySkillCache(skill, 'ElementType', 256)
    cl_action.CreateRandomNumWarriorAtPointPos(skill, cl_action.CreateBoxPosList(skill, cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), (0, cl_action.GetPointTargetModelHeight(skill, cl_action.GetSkillVID(skill)) * 1.5, 0)), (0.8, 0.8, 0.8), 1, 0.3, 0), WARRIOR_SUMMON, {
        1067 if cl_action.CheckSkillRandomInRange(skill, 0, 33) else 1068 if cl_action.CheckSkillRandomInRange(skill, 34, 67) else 1069: 10 }, {
        'Radius': 0.3 })
