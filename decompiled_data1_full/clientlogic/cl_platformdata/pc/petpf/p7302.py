# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/petpf/p7302.pyc
# RelativePath: clientlogic/cl_platformdata/pc/petpf/p7302.pyc
...
class CCartoon0(DirectPosCartoon):
    ...
    def Hit(cls, skill):
        ...
        cl_action.PerformDamage(skill, {
            'Att': cl_action.ToInt(skill, skill.m_Cache['Att'] * ((cl_action.GetSkillServerCache(skill, 'AttSpeedMul') - 2.5 if cl_action.GetSkillServerCache(skill, 'AttSpeedMul') > 2.5 else 0) / 2.5 + 3)) }, dArgs = { })
...
class CCartoon4(TimerCartoon):
    ...
    def Active(cls, skill):
        cl_action.StartBackSwing(skill, cl_action.ToInt(skill, 150 / cl_action.GetSkillVarCache(skill, 'AttackSpeed')))
...
class CCartoon6(MonsterDashCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgCustomPos(skill, cartoon), 0, 0, 21, True, False, False, checkDis = 0, start = (0, 0, 0), offSetY = 0, staticbreak = False, CheckTime = 0, StopDis = 0, CheckHalt = False, targettype = OBJ_ALL, Collision = False)
...
class CCartoon1(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 86 / cl_action.GetSkillVarCache(skill, 'AttackSpeed')), 1)

    InitSuccess = classmethod(InitSuccess)


def Action(skill):
    cl_action.SetSkillVarCache(skill, 'AttackSpeed', cl_action.Clamp(skill, 306 / (skill.m_Cache['AttSpeed'] if skill.m_Cache['AttSpeed'] > 0 else 306), 1, 2.5))
