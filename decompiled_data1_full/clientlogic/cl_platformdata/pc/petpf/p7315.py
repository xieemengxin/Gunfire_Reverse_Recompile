# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/petpf/p7315.pyc
# RelativePath: clientlogic/cl_platformdata/pc/petpf/p7315.pyc
# Source Generated with Decompyle++
# File: p7315.pyc (Python 3.6)
...
class CCartoon2(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 30 / cl_action.GetSkillVarCache(skill, 'AttackSpeed')), 1)
...
class CCartoon0(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 10 / cl_action.GetSkillVarCache(skill, 'AttackSpeed')), 1)
...
class CCartoon4(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 55 / cl_action.GetSkillVarCache(skill, 'AttackSpeed')), 1)

    InitSuccess = classmethod(InitSuccess)


def Action(skill):
    cl_action.SetSkillVarCache(skill, 'AttackSpeed', cl_action.Clamp(skill, 65 / (skill.m_Cache['AttSpeed'] if skill.m_Cache['AttSpeed'] > 0 else 65), 1, 2.5))
    cartoon = { }
