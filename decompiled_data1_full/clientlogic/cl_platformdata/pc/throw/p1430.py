# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/throw/p1430.pyc
# RelativePath: clientlogic/cl_platformdata/pc/throw/p1430.pyc
...
from cl_commondefines import ATT_SHAPE_RECTANGLE, CRT_CHECK_SERVER, SKILLCACHE_INT, SKILLCACHE_PERFORMMODE
        ...
        if cl_action.CheckHasTalent(skill, 3015):
            if cl_action.CrtArgRandomNum(skill, 0, 100) < cl_action.GetAttackerCustomData(skill, 'p3015_Ratio', iDefault = 0):
                if cl_action.CheckHasPerfrom(skill, 6084):
                    cl_action.SetSkillServerCache(skill, '6048AddCount', cl_action.GetTargetStateCount(skill, 33643, cl_action.GetTargetOwnerID(skill, cl_action.GetCurVID(skill)), False, True, bFromWeapon = False))
                    cl_action.AddAttackerStateCount(skill, 33639, (1 + cl_action.GetSkillServerCache(skill, '6048AddCount')) * cl_action.GetAttackerCustomData(skill, 'p3015_Add', iDefault = 1), iTime = 0)
                else:
                    cl_action.SetSkillServerCache(skill, '6048AddCount', 0)
                    cl_action.AddAttackerStateCount(skill, 33639, (1 + cl_action.GetSkillServerCache(skill, '6048AddCount')) * cl_action.GetAttackerCustomData(skill, 'p3015_Add', iDefault = 1), iTime = 0)
            elif cl_action.CheckHasPerfrom(skill, 6084):
                cl_action.SetSkillServerCache(skill, '6048AddCount', cl_action.GetTargetStateCount(skill, 33643, cl_action.GetTargetOwnerID(skill, cl_action.GetCurVID(skill)), False, True, bFromWeapon = False))
                cl_action.AddAttackerStateCount(skill, 33639, 1 + cl_action.GetSkillServerCache(skill, '6048AddCount'), iTime = 0)
            else:
                cl_action.SetSkillServerCache(skill, '6048AddCount', 0)
                cl_action.AddAttackerStateCount(skill, 33639, 1 + cl_action.GetSkillServerCache(skill, '6048AddCount'), iTime = 0)
        elif cl_action.CheckHasPerfrom(skill, 6084):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, (cl_action.GetSkillCustomArg(skill, 'SX') / 100, cl_action.GetSkillCustomArg(skill, 'SY') / 100, cl_action.GetSkillCustomArg(skill, 'SZ') / 100), (cl_action.GetSkillCustomArg(skill, 'EX') / 100, cl_action.GetSkillCustomArg(skill, 'EY') / 100, cl_action.GetSkillCustomArg(skill, 'EZ') / 100), ATT_SHAPE_RECTANGLE, [
                40,
                8,
                6], cl_action.GetAttackerPerformAttr(skill, 1422, 'DamInterval'), 0.25, isUseStartEff = True)
        else:
            cls.EnableCtrl(skill, (cl_action.GetSkillCustomArg(skill, 'SX') / 100, cl_action.GetSkillCustomArg(skill, 'SY') / 100, cl_action.GetSkillCustomArg(skill, 'SZ') / 100), (cl_action.GetSkillCustomArg(skill, 'EX') / 100, cl_action.GetSkillCustomArg(skill, 'EY') / 100, cl_action.GetSkillCustomArg(skill, 'EZ') / 100), ATT_SHAPE_RECTANGLE, [
                40,
                8,
                6], cl_action.GetAttackerPerformAttr(skill, 1422, 'DamInterval'), 0.25, isUseStartEff = True)
