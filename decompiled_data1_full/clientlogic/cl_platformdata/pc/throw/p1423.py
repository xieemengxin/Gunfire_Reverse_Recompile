# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/throw/p1423.pyc
# RelativePath: clientlogic/cl_platformdata/pc/throw/p1423.pyc
...
class CCartoon2(DirectPosCartoon):
    ...
    def Active(cls, skill):
        cl_action.CheckSightCentrePosDis(skill, cl_action.GetStartPositionInCrt(skill, 2), 30)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            ...
            cls.EnableShow(skill, [
                skill.m_Cache['Radius'] * 2 if cl_action.GetSkillVarCache(skill, 'TalentAction') else skill.m_Cache['Radius']], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = True, explosion = False, multipleExplode = True, explosionDelay = 10)
        elif cl_action.GetSkillVarCache(skill, 'TalentAction'):
            pass
        
        skill(cl_action.GetSkillVarCache(skill, 'raypos'), (0, 0, 0), skill.m_Cache['Radius'] * 2, [
...
class CCartoon7(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cartoon = { }
        CCartoon3.Init(skill, cartoon, casting = 0 if cl_action.GetSkillVarCache(skill, 'TalentAction') else 1, index = 10)
...
class CCartoon6(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.AttackerUsePerform(skill, 8007, {
            'TransDamFactor': cl_action.GetTransDamFactor(skill) }, 0, cl_action.GetCartoonHitTargetInOrder(skill, 8))
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            ...
            cls.EnableCtrl(skill, 15, cl_action.GetCartoonHitTargetCount(skill, 8))
...
class CCartoon8(LightningChainCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, 99, 8, 8, 0, '', 0, True)
        else:
            cls.EnableCtrl(skill, False, cl_action.GetStartPositionInCrt(skill, 0), 99, 8, 8, 0, 0, 0, 0, True, [], 0, '', 0, True)
...
class CCartoon0(DirectPosCartoon):
    ...
    def End(cls, skill):
        if cl_action.GetSkillVarCache(skill, 'TalentAction'):
            ...
            cartoon = { }
            CCartoon3.Init(skill, cartoon, casting = 0 if cl_action.GetSkillVarCache(skill, 'TalentAction') else 1, index = 10)
        if not cl_action.CheckHasBenediction(skill, 13528) and cl_action.GetSkillCustomData(skill, 'UseByAI', defaultValue = 0) == 0 and cl_action.CheckHasState(skill, 32701):
...
class CCartoon5(TimerCartoon):
    ...
    def Active(cls, skill):
        cl_action.SetSkillVarCache(skill, 'raypos', cl_action.GetSpherePointByRay(skill, 0.01, 1, 30) if cl_action.GetSkillCustomData(skill, 'UseByAI', defaultValue = 0) == 0 else cl_action.CrtArgTargetPos(skill, notContainDying = True))
