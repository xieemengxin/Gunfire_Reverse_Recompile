# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/throw/p1424.pyc
# RelativePath: clientlogic/cl_platformdata/pc/throw/p1424.pyc
...
import cl_evcon
import cl_war
from cl_commondefines import GAMBLE_TYPE_THROW
...
def Action(skill):
    ...
    cl_action.CacheCountTransDamFactor(skill, 'pf1424', cl_action.GetSkillServerCache(skill, 'CardCnt'))
    ...
    def UsePerform(self, oWarrior, oSkill):
        if oSkill.m_CheckType == CRT_CHECK_SERVER:
            cl_action.SetSkillCacheData(oSkill, SKILLCACHE_LSTINT, [
                3])
            oSkill.m_Collect['ForbidAppendQuality'] = 1
            iTarget = oSkill.m_Base['VID']
            oTarget = oWarrior.m_Game.GetObject(iTarget)
            vEnd = oTarget.GetPos()
            oWarrior.m_Agent.Call_Out(Functor(DelayUseSubPF, oWarrior, iTarget, vEnd, 0, 3), 15, 'AIThrowPF')
...
def DelayUseSubPF(oWarrior, iTarget, vEnd, iCur, iTotal):
    ...
    if not cl_war.UsePerform(oWarrior, oPerform, dData):
        return None
    if iCur < iTotal:
        oWarrior.m_Agent.Call_Out(Functor(DelayUseSubPF, oWarrior, iTarget, vEnd, iCur, iTotal), 3, 'AIThrowPF')
