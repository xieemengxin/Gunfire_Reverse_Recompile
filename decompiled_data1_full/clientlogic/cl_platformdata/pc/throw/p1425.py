# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/throw/p1425.pyc
# RelativePath: clientlogic/cl_platformdata/pc/throw/p1425.pyc
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, LEVEL_TYPE_BOSS, OBJ_ENEMY, SKILLCACHE_LSTINT, SKILLCACHE_LSTINTSPECIAL, SKILLCACHE_LSTPOS
...
class CCartoon0(DirectPosCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetEndPositionInCrt(skill, 6), end = (0, 0, 0)):
                return None
...
class CCartoon6(TraceCartoon):
    ...
    def Hit(cls, skill):
        if cl_action.GetSkillCustomData(skill, 'UseByAI', defaultValue = 0) == 0 and cl_action.CheckHasBenediction(skill, 13544):
            cartoon = { }
    ...
    def InitSuccess(cls, skill, i1, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTPOS)[i1]):
                return None
            cls.EnableShow(skill, skill.m_Cache['Pierce'], 100 if cl_action.CheckCurLevelType(skill, LEVEL_TYPE_BOSS) else 50, 30, showstart = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTPOS)[i1], targettype = OBJ_ENEMY, liveTime = 0, lineDistance = 1, angle = 100, lockDis = 35, IgnoreDefalutDis = 0, iIgnoreMonsterID = 0, FilterDie = True, lockFromCartoon = True, isMultipleBeacon = False, targetOffset = (0, 0, 0), arriveDoTrigger = False, accelerated = 0)
        elif cl_action.CheckCurLevelType(skill, LEVEL_TYPE_BOSS):
            pass
        
        skill(cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTPOS)[i1], cl_action.GetPointOnAimPlane(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTPOS)[i1], cl_action.GetCameraDirPos(skill, -20), cl_action.GetSceneCenterPosition(skill, cartoon)), skill.m_Cache['Pierce'], 100, 50, 30, showstart = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTPOS)[i1], targettype = OBJ_ENEMY, targetID = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[i1], liveTime = 0, lineDistance = 1, angle = 100, lockWeakness = False, lockAngle = 19, canDesAngle = 0, lockDis = 35, IgnoreDefalutDis = 0, IgnoreSummon = False, iIgnoreMonsterID = 0, FilterDie = True, lockFromCartoon = True, isMultipleBeacon = False, targetOffset = (0, 0, 0), arriveDoTrigger = False, accelerated = 0, lockSumm = False, lockCanDestroy = False, lockHideDoor = False, isBlocked = False, isMonsterFirst = False, lockExplode = False)

    InitSuccess = classmethod(InitSuccess)


def Action(skill):
    cl_action.ApplyCountTransDamFactor(skill, 'pf1424')
...
class CPerform(CCustomPerform):
    ...
    def UsePerform(self, oWarrior, oSkill):
        if oSkill.m_CheckType == CRT_CHECK_SERVER:
            ...
            vPos = oAttack.GetPos()
            vPos = (vPos[0], vPos[1] + oAttack.m_ModelHeight * 0.6, vPos[2])
            ...
            cl_action.SetSkillCacheData(oSkill, SKILLCACHE_LSTINT, [
                oSkill.m_Base['VID']])
        iUseCardCnt = len(cl_action.GetSkillCacheData(oSkill, SKILLCACHE_LSTINTSPECIAL))
