# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/passive/p51224.pyc
# RelativePath: clientlogic/cl_platformdata/pc/passive/p51224.pyc
# Source Generated with Decompyle++
# File: p51224.pyc (Python 3.6)
...
def DoCallBackAction1(oEventCB, oWarrior):
    if cl_condition.GetWeaponBulletCnt(oWarrior, oEventCB.GetCBLifeCycle()) > 0 and cl_evcon.CheckFromPointPerform(oWarrior, oEventCB, 97091, 0, None):
    ...
    if cl_evcon.CheckFromPointPerform(oWarrior, oEventCB, 97011, 1, None):
        cl_evact.EventChangeSkillCache(oWarrior, oEventCB, 'DebuffProb', 7000, 0)


def DoCallBackAction2(oEventCB, oWarrior):
    if cl_evcon.CheckFromPointPerform(oWarrior, oEventCB, 97091, 1, None) and cl_evcon.CheckVictimFightType(oWarrior, oEventCB, WARRIOR_NORMAL):
...
'sAttr': 'Shield' })) * 25 / 100), DAM_TYPE_WEAPON | DAM_TYPE_TRUE | DAM_USE_ALL, 1, 0, 0, 0, None, None, None, None, None, None, None)
    if cl_evcon.CheckFromPointPerform(oWarrior, oEventCB, 97091, 1, None) and cl_evcon.CheckVictimFightType(oWarrior, oEventCB, WARRIOR_ELITE):
        cl_evact.EventGetTargetByType(oWarrior, oEventCB, OBJ_VICTIM)
        cl_evact.EventTargetDamage(oWarrior, oEventCB, (lambda *a: (Func302(*a, **{
'sAttr': 'HP' }) + Func302(*a, **{
'sAttr': 'Armor' }) + Func302(*a, **{
'sAttr': 'Shield' })) * 12 / 100), DAM_TYPE_WEAPON | DAM_TYPE_TRUE | DAM_USE_ALL, 1, 0, 0, 0, None, None, None, None, None, None, None)
    if cl_evcon.CheckFromPointPerform(oWarrior, oEventCB, 97091, 1, None) and cl_evcon.CheckVictimFightType(oWarrior, oEventCB, WARRIOR_BOSS):
        cl_evact.EventGetTargetByType(oWarrior, oEventCB, OBJ_VICTIM)
        cl_evact.EventTargetDamage(oWarrior, oEventCB, (lambda *a: (Func302(*a, **{
'sAttr': 'HP' }) + Func302(*a, **{
'sAttr': 'Armor' }) + Func302(*a, **{
