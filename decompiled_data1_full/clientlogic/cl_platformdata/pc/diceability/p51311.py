# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/diceability/p51311.pyc
# RelativePath: clientlogic/cl_platformdata/pc/diceability/p51311.pyc
# Source Generated with Decompyle++
# File: p51311.pyc (Python 3.6)
...
from cl_commondefines import ATTACKERSUBMSG_NORMAL, DICETAG_PERFORM, DICE_PUTOUT_POLL_ONE, OBJ_ATTACK, PF_TYPE_THROW
...
def Action4(oWarrior, oLifeCycle):
    if cl_condition.CheckHero(oWarrior, oLifeCycle, 214) or cl_condition.CheckHero(oWarrior, oLifeCycle, 207):
        cl_action.CommonChangeThrowPerformAttr(oWarrior, oLifeCycle, 'DamInterval', 6000, 0, None)
    else:
        cl_action.CommonChangeThrowPerformAttr(oWarrior, oLifeCycle, 'Radius', 6000, 0, None)
    if cl_condition.CheckHero(oWarrior, oLifeCycle, 219) or cl_condition.CheckHero(oWarrior, oLifeCycle, 207):
        cl_action.CommonListenMsgCallBack(oWarrior, oLifeCycle, cl_msgcenter.MSG_WAR_PERFORM, ATTACKERSUBMSG_NORMAL, 0, 0, 0)
    else:
        cl_action.CommonListenMsgCallBack(oWarrior, oLifeCycle, cl_msgcenter.MSG_WAR_PERFORM, ATTACKERSUBMSG_NORMAL, 3, 0, 0)
...
def Action5(oWarrior, oLifeCycle):
    if cl_condition.CheckHero(oWarrior, oLifeCycle, 214) or cl_condition.CheckHero(oWarrior, oLifeCycle, 207):
        cl_action.CommonChangeThrowPerformAttr(oWarrior, oLifeCycle, 'DamInterval', 10000, 0, None)
    else:
        cl_action.CommonChangeThrowPerformAttr(oWarrior, oLifeCycle, 'Radius', 10000, 0, None)
    if cl_condition.CheckHero(oWarrior, oLifeCycle, 219) or cl_condition.CheckHero(oWarrior, oLifeCycle, 207):
        cl_action.CommonListenMsgCallBack(oWarrior, oLifeCycle, cl_msgcenter.MSG_WAR_PERFORM, ATTACKERSUBMSG_NORMAL, 0, 0, 0)
    else:
