# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/attack/p9608.pyc
# RelativePath: clientlogic/cl_platformdata/pc/attack/p9608.pyc
# Source Generated with Decompyle++
# File: p9608.pyc (Python 3.6)
...
class CCartoon2(ThrowByPowerCartoon):
    ...
    def Hit(cls, skill):
        ...
        if cl_action.GetAttackerWeaponPerformAttr(skill, 9608, 'MinUseEnergy') >= 1 and cl_action.GetSkillServerCache(skill, 'HadRecover1') < 1:
            cl_action.FillPFBullet(skill, 9690, cl_action.GetAttackerWeaponPerformAttr(skill, 9608, 'PFBulletRecover'), 1)
    ...
    def HitStatic(cls, skill):
        if cl_action.GetSkillVarCache(skill, 'isHitStatic') and cl_math.CalDistance3D(cl_action.CrtArgHitPos(skill), cl_action.GetStartPositionInCrt(skill, 2)) <= 4.8 and cl_action.IsHeroCtrl(skill):
...
class CCartoon9(ThrowByPowerCartoon):
    m_SID = 9
    ...
    def Hit(cls, skill):
        ...
        if cl_action.CheckHasInscription(skill, 13121) and cl_action.GetSkillServerCache(skill, 'HadRecover2') < 1:
            cl_action.AttackerAddState(skill, 33927, 1000, 0, { })
            cl_action.AddAttackerStateCount(skill, 33927, 1, iTime = 0, bFromAttack = False, bFromWeapon = False)
        if cl_action.GetAttackerWeaponPerformAttr(skill, 9608, 'MinUseEnergy') >= 1 and cl_action.GetSkillServerCache(skill, 'HadRecover2') < 1:
            cl_action.FillPFBullet(skill, 9690, cl_action.GetAttackerWeaponPerformAttr(skill, 9608, 'PFBulletRecover'), 1)
    ...
    def HitStatic(cls, skill):
        if cl_action.GetSkillVarCache(skill, 'isHitStatic') and cl_math.CalDistance3D(cl_action.CrtArgHitPos(skill), cl_action.GetStartPositionInCrt(skill, 9)) <= 4.8 and cl_action.IsHeroCtrl(skill):
            cl_action.SetSkillVarCache(skill, 'isHitStatic', True)
...
class CCartoon6(MeleeWeaponCartoon):
    m_SID = 6
    ...
    def End(cls, skill):
        if cl_action.GetCartoonHitTargetCount(skill, 6) >= 1:
            if cl_action.CheckHasInscription(skill, 13121):
                cl_action.AttackerAddState(skill, 33927, 1000, 0, { })
                cl_action.AddAttackerStateCount(skill, 33927, 1, iTime = 0, bFromAttack = False, bFromWeapon = False)
            if not cl_action.GetAttackerWeaponPerformAttr(skill, 9608, 'MinUseEnergy') >= 1:
                cl_action.FillPFBullet(skill, 9690, cl_action.GetAttackerWeaponPerformAttr(skill, 9608, 'PFBulletRecover'), 1)
    ...
    def Hit(cls, skill):
        if cl_action.IsHeroCtrl(skill):
            cl_action.WeaponDamage(skill, {
                'Att': cl_action.GetAttackerWeaponPerformAttr(skill, 9608, 'Att') }, { }, sendPFMsg = False)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            ...
            cls.EnableShow(skill, [
                ...
                0.4,
                100,
                -15], meshCount = 5, totalTime = 10, targettype = OBJ_ENEMY, pierceStatic = False, left2right = True, center2around = False, canCritical = False)
...
class CCartoon7(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, cl_action.ToInt(skill, 4 if 8 / cl_action.GetAttSpeedRatio(skill) < 4 else 8 / cl_action.GetAttSpeedRatio(skill) - 8 / cl_action.GetAttSpeedRatio(skill) % 4), 1)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            ...
            cls.EnableShow(skill, [
                ...
                0.4,
                100,
                30], meshCount = 5, totalTime = 10, targettype = OBJ_ENEMY, pierceStatic = False, left2right = True, center2around = False, canCritical = False)
