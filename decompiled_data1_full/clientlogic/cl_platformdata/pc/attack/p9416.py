# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/attack/p9416.pyc
# RelativePath: clientlogic/cl_platformdata/pc/attack/p9416.pyc
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, CRT_EXTCHECK_LASERWEAPON, CRT_EXTCHECK_NONE, MONSTER_PART_BARRIAR, MONSTER_PART_SHIELD, OBJ_ENEMY, SKILLCACHE_INT
...
class CCartoon0(DirectPosCartoon):
    ...
    def Hit(cls, skill):
        cl_action.PushVictim(skill, cl_action.GetEndPositionInCrt(skill, 0), 20, 5, 8000, angle = 0, iCartoonSID = -1, iIgnoreStruckCD = 0, iClient = 0)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.CrtArgHitPos(skill) if cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) <= 1 else cl_action.GetTargetCenterPos(skill, cl_action.GetCartoonHitTargetInOrder(skill, 4)), end = (0, 0, 0)):
                return None
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetCameraCenterPosition(skill, cartoon)):
                return None
            cls.EnableShow(skill, skill.m_Cache['Pierce'], cl_action.GetWeaponAttDis(skill), skill.m_Cache['BulletSpeed'], targettype = OBJ_ENEMY, liveTime = 0, radius = 1.5, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon4(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetCameraCenterPosition(skill, cartoon)):
                return None
            cls.EnableShow(skill, 10, cl_action.GetWeaponAttDis(skill), skill.m_Cache['BulletSpeed'], targettype = OBJ_ENEMY, liveTime = 0, radius = 1.5, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_LASERWEAPON)
...
def Action(skill):
    if cl_action.IsHeroCtrl(skill):
        cl_action.SetSkillVarCache(skill, 'EnergyBarCount', skill.m_Cache['EnergyBar'])
