# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/attack/p9514.pyc
# RelativePath: clientlogic/cl_platformdata/pc/attack/p9514.pyc
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetEndPositionInCrt(skill, 3)):
                return None
            cls.EnableShow(skill, 1, 50, 50, showstart = (0, 0, 0), targettype = OBJ_ENEMY, liveTime = 0, lineDistance = 0, angle = 180, lockDis = 0, IgnoreDefalutDis = 0, iIgnoreMonsterID = cl_action.GetCartoonHitTargetInOrder(skill, 3), FilterDie = True, lockFromCartoon = False, isMultipleBeacon = False, targetOffset = (0, 0, 0), arriveDoTrigger = False, accelerated = 0)
...
class CCartoon7(TraceCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetEndPositionInCrt(skill, 4)):
                return None
            cls.EnableShow(skill, 1, 50, 50, showstart = (0, 0, 0), targettype = OBJ_ENEMY, liveTime = 0, lineDistance = 0, angle = 180, lockDis = 0, IgnoreDefalutDis = 0, iIgnoreMonsterID = cl_action.GetCartoonHitTargetInOrder(skill, 4), FilterDie = True, lockFromCartoon = False, isMultipleBeacon = False, targetOffset = (0, 0, 0), arriveDoTrigger = False, accelerated = 0)
...
class CCartoon2(LightningChainCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, 2, 10, 10, cl_action.GetCartoonHitTargetInOrder(skill, 4), '', 1, True)
...
class CCartoon4(TraceCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetEndPositionInCrt(skill, 3)):
                return None
            cls.EnableShow(skill, 1, 50, 30, showstart = (0, 0, 0), targettype = OBJ_ENEMY, liveTime = 0, lineDistance = 0, angle = 180, lockDis = 0, IgnoreDefalutDis = 0, iIgnoreMonsterID = cl_action.GetCartoonHitTargetInOrder(skill, 3), FilterDie = True, lockFromCartoon = False, isMultipleBeacon = False, targetOffset = (0, 0, 0), arriveDoTrigger = False, accelerated = 0)
...
class CCartoon0(LightningChainCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, 2 if cl_action.CheckHasInscription(skill, 13040) else 1, 10, 10, cl_action.GetCartoonHitTargetInOrder(skill, 3), '', 1, True)
    ...
    def InitSuccess(cls, skill, i1, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetMuzzlePosition(skill, cartoon)):
                return None
            cls.EnableShow(skill, 0.4, (0, 16, 0), (0.99, 0.99), 300, True, True, 0, innerRadius = 0.1, pierce = 1, ignoreMonster = False, hitUnitBounciness = (0, 0), targettype = OBJ_ENEMY, hitovertype = HIT_OVER_NORMAL, changeRadius = 0, maxRadius = 3, iTriggerBullet = 0)
