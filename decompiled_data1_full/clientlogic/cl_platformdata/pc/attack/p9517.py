# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/attack/p9517.pyc
# RelativePath: clientlogic/cl_platformdata/pc/attack/p9517.pyc
    ...
    def InitSuccess(cls, skill, i2, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetCameraCenterPosition(skill, cartoon)):
                return None
            cls.EnableShow(skill, skill.m_Cache['Pierce'], cl_action.GetWeaponAttDis(skill), 120, targettype = OBJ_ENEMY, liveTime = 0, radius = (cl_action.GetCartoonChargeLevel(skill, 2) - 3) * 0.2 + 0.45, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_PENETRATION)
        ...
        if cl_action.IsOpenSnipe(skill):
            if cl_action.CheckHasInscription(skill, 4896):
                for i5 in range(0, cl_action.GetTrajectory(skill), 1):
                    cartoon = { }
                    CCartoon0.Init(skill, cartoon, casting = 0, index = i5)
...
class CCartoon2(ChargeCartoon):
    ...
    def Trigger(cls, skill):
        if cl_action.IsHeroCtrl(skill):
            if cl_action.GetCartoonChargeLevel(skill, 2) == 2:
                if cl_action.GetCartoonChargeLevel(skill, 2) >= 3:
                    cl_action.ModifySkillCache(skill, 'CrazyEff', (cl_action.GetCartoonChargeLevel(skill, 2) - 3) * 10000 + cl_action.GetWeaponIntAttr(skill, 'CrazyEff'))
                if cl_action.GetPerformArgValue(skill, 'CalLucky', iDefault = 0) > 0:
                    cl_action.ModifySkillCache(skill, 'LuckyHit', 14 * cl_action.GetCartoonChargeLevel(skill, 2) + skill.m_Cache['LuckyHit'])
                if cl_action.IsOpenSnipe(skill):
                    if cl_action.CheckHasInscription(skill, 4896):
                ...
                if cl_action.IsOpenSnipe(skill):
                    if cl_action.CheckHasInscription(skill, 4896):
                        for i9 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon0.Init(skill, cartoon, casting = 0, index = i9)
                        
                    else:
                        cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                        for i10 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon1.Init(skill, cartoon, casting = 0, index = i10)
                        
                elif cl_action.CheckHasInscription(skill, 4896):
                    for i11 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon0.Init(skill, cartoon, casting = 0, index = i11)
                    
                else:
                    cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                    for i12 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon1.Init(skill, cartoon, casting = 0, index = i12)
                    
                if cl_action.GetAttackerAttr(skill, 'BurstCount') > 0:
                    cartoon = { }
                    CCartoon6.Init(skill, cartoon, casting = 0, index = 0)
                
            if cl_action.GetCartoonChargeLevel(skill, 2) == 4:
                if cl_action.GetCartoonChargeLevel(skill, 2) >= 3:
                    cl_action.ModifySkillCache(skill, 'CrazyEff', (cl_action.GetCartoonChargeLevel(skill, 2) - 3) * 10000 + cl_action.GetWeaponIntAttr(skill, 'CrazyEff'))
                if cl_action.GetPerformArgValue(skill, 'CalLucky', iDefault = 0) > 0:
                    cl_action.ModifySkillCache(skill, 'LuckyHit', 14 * cl_action.GetCartoonChargeLevel(skill, 2) + skill.m_Cache['LuckyHit'])
                if cl_action.IsOpenSnipe(skill):
                    if cl_action.CheckHasInscription(skill, 4896):
                        for i17 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon0.Init(skill, cartoon, casting = 0, index = i17)
                        
                    else:
                        cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                        for i18 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon1.Init(skill, cartoon, casting = 0, index = i18)
                        
                elif cl_action.CheckHasInscription(skill, 4896):
                    for i19 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon0.Init(skill, cartoon, casting = 0, index = i19)
                    
                else:
                    cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                    for i20 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon1.Init(skill, cartoon, casting = 0, index = i20)
                    
                if cl_action.GetAttackerAttr(skill, 'BurstCount') > 0:
                    cartoon = { }
                    CCartoon6.Init(skill, cartoon, casting = 0, index = 0)
                
            if cl_action.GetCartoonChargeLevel(skill, 2) == 5:
                if cl_action.GetCartoonChargeLevel(skill, 2) >= 3:
                    cl_action.ModifySkillCache(skill, 'CrazyEff', (cl_action.GetCartoonChargeLevel(skill, 2) - 3) * 10000 + cl_action.GetWeaponIntAttr(skill, 'CrazyEff'))
                if cl_action.GetPerformArgValue(skill, 'CalLucky', iDefault = 0) > 0:
                    cl_action.ModifySkillCache(skill, 'LuckyHit', 14 * cl_action.GetCartoonChargeLevel(skill, 2) + skill.m_Cache['LuckyHit'])
                if cl_action.IsOpenSnipe(skill):
                    if cl_action.CheckHasInscription(skill, 4896):
                        for i25 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon0.Init(skill, cartoon, casting = 0, index = i25)
                        
                    else:
                        cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                        for i26 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon1.Init(skill, cartoon, casting = 0, index = i26)
                        
                elif cl_action.CheckHasInscription(skill, 4896):
                    for i27 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon0.Init(skill, cartoon, casting = 0, index = i27)
                    
                else:
                    cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                    for i28 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon1.Init(skill, cartoon, casting = 0, index = i28)
                    
                if cl_action.GetAttackerAttr(skill, 'BurstCount') > 0:
                    cartoon = { }
                    CCartoon6.Init(skill, cartoon, casting = 0, index = 0)
                
            if cl_action.GetCartoonChargeLevel(skill, 2) == 6:
                if cl_action.GetCartoonChargeLevel(skill, 2) >= 3:
                    cl_action.ModifySkillCache(skill, 'CrazyEff', (cl_action.GetCartoonChargeLevel(skill, 2) - 3) * 10000 + cl_action.GetWeaponIntAttr(skill, 'CrazyEff'))
                if cl_action.GetPerformArgValue(skill, 'CalLucky', iDefault = 0) > 0:
                    cl_action.ModifySkillCache(skill, 'LuckyHit', 14 * cl_action.GetCartoonChargeLevel(skill, 2) + skill.m_Cache['LuckyHit'])
                if cl_action.IsOpenSnipe(skill):
                    if cl_action.CheckHasInscription(skill, 4896):
                        for i33 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon0.Init(skill, cartoon, casting = 0, index = i33)
                        
                    else:
                        cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                        for i34 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon1.Init(skill, cartoon, casting = 0, index = i34)
                        
                elif cl_action.CheckHasInscription(skill, 4896):
                    for i35 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon0.Init(skill, cartoon, casting = 0, index = i35)
                    
                else:
                    cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                    for i36 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon1.Init(skill, cartoon, casting = 0, index = i36)
                    
                if cl_action.GetAttackerAttr(skill, 'BurstCount') > 0:
                    cartoon = { }
                    CCartoon6.Init(skill, cartoon, casting = 0, index = 0)
                
            if cl_action.GetCartoonChargeLevel(skill, 2) == 7:
                if cl_action.GetCartoonChargeLevel(skill, 2) >= 3:
                    cl_action.ModifySkillCache(skill, 'CrazyEff', (cl_action.GetCartoonChargeLevel(skill, 2) - 3) * 10000 + cl_action.GetWeaponIntAttr(skill, 'CrazyEff'))
                if cl_action.GetPerformArgValue(skill, 'CalLucky', iDefault = 0) > 0:
                    cl_action.ModifySkillCache(skill, 'LuckyHit', 14 * cl_action.GetCartoonChargeLevel(skill, 2) + skill.m_Cache['LuckyHit'])
                if cl_action.IsOpenSnipe(skill):
                    if cl_action.CheckHasInscription(skill, 4896):
                        for i41 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon0.Init(skill, cartoon, casting = 0, index = i41)
                        
                    else:
                        cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                        for i42 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon1.Init(skill, cartoon, casting = 0, index = i42)
                        
                elif cl_action.CheckHasInscription(skill, 4896):
                    for i43 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon0.Init(skill, cartoon, casting = 0, index = i43)
                    
                else:
                    cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                    for i44 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon1.Init(skill, cartoon, casting = 0, index = i44)
                    
                if cl_action.GetAttackerAttr(skill, 'BurstCount') > 0:
                    cartoon = { }
                    CCartoon6.Init(skill, cartoon, casting = 0, index = 0)
                elif cl_action.GetCartoonChargeLevel(skill, 2) >= 3:
                    cl_action.ModifySkillCache(skill, 'CrazyEff', (cl_action.GetCartoonChargeLevel(skill, 2) - 3) * 10000 + cl_action.GetWeaponIntAttr(skill, 'CrazyEff'))
                if cl_action.GetPerformArgValue(skill, 'CalLucky', iDefault = 0) > 0:
                    cl_action.ModifySkillCache(skill, 'LuckyHit', 14 * cl_action.GetCartoonChargeLevel(skill, 2) + skill.m_Cache['LuckyHit'])
                if cl_action.IsOpenSnipe(skill):
                    if cl_action.CheckHasInscription(skill, 4896):
                        for i49 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon0.Init(skill, cartoon, casting = 0, index = i49)
                        
                    else:
                        cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                        for i50 in range(0, cl_action.GetTrajectory(skill), 1):
                            cartoon = { }
                            CCartoon1.Init(skill, cartoon, casting = 0, index = i50)
                        
                elif cl_action.CheckHasInscription(skill, 4896):
                    for i51 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon0.Init(skill, cartoon, casting = 0, index = i51)
                    
                else:
                    cl_action.SetSkillVarCache(skill, 'BulletFrameStart', cl_action.GetMuzzlePos(skill))
                    for i52 in range(0, cl_action.GetTrajectory(skill), 1):
                        cartoon = { }
                        CCartoon1.Init(skill, cartoon, casting = 0, index = i52)
