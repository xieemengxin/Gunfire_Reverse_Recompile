# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/careerpf/p1319.pyc
# RelativePath: clientlogic/cl_platformdata/pc/careerpf/p1319.pyc
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, OBJ_ENEMY, SKILLCACHE_EXTRATRAJECTORY, SKILLCACHE_INT, SKILLCACHE_LSTINT, SKILLCACHE_LSTINTSPECIAL
...
class CCartoon2(RangeSpreadCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, cl_action.GetPosInCustomData(skill), cl_action.GetCustomPid(skill), skill.m_Cache['DamInterval'], skill.m_Cache['AddStateTime'], 13 if cl_action.GetTalentLevel(skill, 3001) == 3 else 12 if cl_action.GetTalentLevel(skill, 3001) == 2 else 11 if cl_action.GetTalentLevel(skill, 3001) == 1 else 10, 1.2, 0.2, 0.9, 1.7)
        else:
            cls.EnableCtrl(skill, cl_action.GetPosInCustomData(skill), cl_action.GetCustomPid(skill), skill.m_Cache['DamInterval'], skill.m_Cache['AddStateTime'], 13 if cl_action.GetTalentLevel(skill, 3001) == 3 else 12 if cl_action.GetTalentLevel(skill, 3001) == 2 else 11 if cl_action.GetTalentLevel(skill, 3001) == 1 else 10, 1.2, 0.2, 0.9, 1.7)
