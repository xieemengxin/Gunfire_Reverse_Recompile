# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/careerpf/p7175.pyc
# RelativePath: clientlogic/cl_platformdata/pc/careerpf/p7175.pyc
...
class CCartoon0(TimerCartoon):
    ...
    def Trigger(cls, skill):
        if cl_action.GetMonsterPhase(skill) == 2:
            cl_action.CustomPerformAction(skill, 7175, 'CustomSkillEnd', {
...
class CCartoon3(DelegateDirectPosCartoon):
    ...
    def Active(cls, skill):
        ...
        cl_action.SkillHaltTargetPerform(skill, cl_action.GetAttackerOwnerID(skill), 8507)
        cartoon = { }
    ...
    def Hit(cls, skill):
        cl_action.PerformDamage(skill, {
            'Att': cl_action.GetAttackerAttr(skill, 'Att') * 4.5 * (cl_action.GetSummonOwnerTalentLevel(skill, 3304) * 1.5 + 1) }, dArgs = { })
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), (0, 0.3, 0)), (0, 0, 0), [
                15], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, useclientpos = False)
