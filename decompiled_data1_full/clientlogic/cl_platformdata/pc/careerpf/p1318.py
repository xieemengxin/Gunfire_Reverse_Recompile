# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/careerpf/p1318.pyc
# RelativePath: clientlogic/cl_platformdata/pc/careerpf/p1318.pyc
...
class CCartoon3(RayCastingPosCartoon):
    ...
    def Hit(cls, skill):
        if cl_action.GetTalentLevel(skill, 3113) == 3:
            if cl_action.CountDistance(skill, cl_action.GetStartPositionInCrt(skill, 3), cl_action.GetCartoonCurPos(skill, 3)) * 0.02 < 1:
                pass
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.VectorShiftForAttDir(skill, cl_action.GetCameraCenterPosition(skill, cartoon), (0, 0, 1.5))):
                return None
            ...
            cls.EnableCtrl(skill, cl_action.VectorShiftForAttDir(skill, cl_action.GetCameraCenterPosition(skill, cartoon), (0, 0, 1.5)), cl_action.GetSceneCenterPosition(skill, cartoon), 99, 50, 30, 1, [
                3,
                6,
                4], attshape = ATT_SHAPE_RECTANGLE, targettype = OBJ_ENEMY, liveTime = 0)
