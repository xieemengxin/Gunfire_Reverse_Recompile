# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/careerpf/p7148.pyc
# RelativePath: clientlogic/cl_platformdata/pc/careerpf/p7148.pyc
# Source Generated with Decompyle++
# File: p7148.pyc (Python 3.6)
...
from cl_commondefines import ATT_SHAPE_SECTOR, CRT_CHECK_SERVER, OBJ_ENEMY, SKILLCACHE_INT, SKILLCACHE_LSTINT, SKILLCACHE_LSTINTSPECIAL
...
class CCartoon4(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, 4 if int(2500 / cl_action.GetSkillCacheData(skill, SKILLCACHE_INT)) < 4 else int(2500 / cl_action.GetSkillCacheData(skill, SKILLCACHE_INT)), 1)
...
class CCartoon0(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, 4 if int(8000 / cl_action.GetSkillCacheData(skill, SKILLCACHE_INT)) < 4 else int(8000 / cl_action.GetSkillCacheData(skill, SKILLCACHE_INT)), 1)
...
class CCartoon5(DelegateDirectPosCartoon):
    ...
    def Hit(cls, skill):
        cl_action.PerformDamage(skill, {
            'Att': cl_action.GetAttackerAttr(skill, 'Att') * 2 })
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), (0, -1.5, 0)), cl_action.CrtArgSkillEndPos(skill), [
                ...
                (cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[0] / cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINTSPECIAL)[0]) * 4,
                90], attshape = ATT_SHAPE_SECTOR, targettype = OBJ_ENEMY, pierceStatic = True, explosion = False, useclientpos = False)
...
def Action(skill):
    ...
    cl_action.SetSkillCacheData(skill, SKILLCACHE_LSTINT, [
        cl_action.GetAttackerPerformAttr(skill, 7148, 'AttDistance')])
    cl_action.SetSkillCacheData(skill, SKILLCACHE_LSTINTSPECIAL, [
        cl_action.GetSkillBaseAttr(skill, 'AttDistance')])
