# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/careerpf/p1324.pyc
# RelativePath: clientlogic/cl_platformdata/pc/careerpf/p1324.pyc
...
class CCartoon5(SendDataCartoon):
    ...
    def End(cls, skill):
        if cl_action.GetSkillCacheData(skill, SKILLCACHE_PERFORMMODE) == 2 and cl_action.GetPerformArgValue(skill, 'SlaySubCD', iDefault = 0) == 1:
            cl_action.SubAttackerCareerPerformColdTime(skill, 40, iTime = 0)
...
class CCartoon3(AnnulusDiffuseCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, cl_action.GetAttackerPerformAttr(skill, 1324, 'Radius') + 4, cl_action.GetAttackerPerformAttr(skill, 1324, 'Radius') + 4 + 12, 20, 1.5, targettype = OBJ_ENEMY, liveTime = 0, pierceStatic = True)
        else:
            cls.EnableCtrl(skill, cl_action.GetStartPositionInCrt(skill, 1), cl_action.GetAttackerPerformAttr(skill, 1324, 'Radius') + 4, cl_action.GetAttackerPerformAttr(skill, 1324, 'Radius') + 4 + 12, 20, 1.5, targettype = OBJ_ENEMY, liveTime = 0, pierceStatic = True)
...
class CCartoon6(TimerCartoon):
    ...
    def End(cls, skill):
        cl_action.SkillForbid(skill, False, 1012)
...
class CCartoon4(TimerCartoon):
    ...
    def Active(cls, skill):
        cl_action.SkillForbid(skill, True, 1012)
        cl_action.SetCurVictim(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[0])
        if cl_action.CheckHasState(skill, 33007) and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) == 360 and cl_action.CheckHasState(skill, 33007):
            if cl_action.GetAttackerStateCount(skill, 33007, dState = { }) >= 180 and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) < 360:
                pass
        ...
        if (95) < 95:
            ...
            cl_action.GetCurVID(skill)(1854, 17, 95, 95 + 10)
            if cl_action.CheckHasState(skill, 33007) and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) == 360 and cl_action.CheckHasState(skill, 33007):
                if cl_action.GetAttackerStateCount(skill, 33007, dState = { }) >= 180 and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) < 360:
                    pass
            ...
            if (95) < 95:
                if not cl_action.CheckMonsterType(skill, WARRIOR_ELITE, cl_action.GetCurVID(skill)) or cl_action.CheckMonsterType(skill, WARRIOR_BOSS, cl_action.GetCurVID(skill)):
                    cl_action.TargetSetStateTime(skill, cl_action.GetCurVID(skill), 1864, (17 if cl_action.CheckHasState(skill, 33007) and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) == 360 else 55 if cl_action.GetAttackerStateCount(skill, 33007, dState = { }) >= 180 and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) < 360 else 95 if cl_action.CheckHasState(skill, 33007) else 95) + 10)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            ...
            17(95, 95, 1)
        elif cl_action.CheckHasState(skill, 33007) and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) == 360 and cl_action.CheckHasState(skill, 33007):
            if cl_action.GetAttackerStateCount(skill, 33007, dState = { }) >= 180 and cl_action.GetAttackerStateCount(skill, 33007, dState = { }) < 360:
                pass
...
def Action(skill):
    ...
    cl_action.CachePFTransDamFactor(skill, 150)
    cartoon = { }
    ...
    if cl_action.GetSkillCacheData(skill, SKILLCACHE_PERFORMMODE) == 2:
        ...
        32949(17, 95, 95 + 10, 1, { }, cl_action.GetCurVID(skill))
        ...
        32958(17, 95, 95, 1, { }, cl_action.GetCurVID(skill))
    cartoon = { }
    ...
    def UsePerform(self, oWarrior, oSkill):
        if oSkill.m_CheckType == CRT_CHECK_SERVER:
            cl_action.SetSkillCacheData(oSkill, SKILLCACHE_LSTINT, [
                oSkill.m_Base['VID']])
            cl_action.SetSkillCacheData(oSkill, SKILLCACHE_POS, oSkill.m_Base['vEnd'])
            cl_action.SetSkillCacheData(oSkill, SKILLCACHE_PERFORMMODE, 2)
        super().UsePerform(oWarrior, oSkill)

    
    def SendUseMsg(self, oWarrior, oSkill):
        lstCache = cl_action.GetSkillCacheData(oSkill, SKILLCACHE_LSTINT)
        iCurVID = lstCache[0]
