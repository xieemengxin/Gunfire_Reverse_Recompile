# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/state/st32619.pyc
# RelativePath: clientlogic/cl_platformdata/pc/state/st32619.pyc
# Source Generated with Decompyle++
# File: st32619.pyc (Python 3.6)
...
def StateCountAction(oTarget, oLifeCycle):
    if cl_condition.StateGetSelfCount(oTarget, oLifeCycle) == 0:
        cl_action.CommonStateStatistics(oTarget, oLifeCycle, 32619, 1, 'st32619_lasthit')
...
'sid': 3016 })))
        if not cl_evcon.CheckPerformType(oTarget, oEventCB, PF_TYPE_CAREERPF, None):
            if cl_evcon.CheckTalentLevel(oTarget, oEventCB, 3016) >= 3:
                cl_action.StateAddState(oTarget, oEventCB.GetCBLifeCycle(), 32720, 200, { }, None)
                cl_evact.StateCBSelfRemove(oTarget, oEventCB)
            else:
                cl_action.StateAddState(oTarget, oEventCB.GetCBLifeCycle(), 32720, 300, { }, None)
                cl_evact.StateCBSelfRemove(oTarget, oEventCB)
        elif cl_evcon.CheckStateStatistics(oTarget, oEventCB, 32619, 'st32619_lasthit') == 1:
            cl_evact.EventCBChangeLuckyHit(oTarget, oEventCB, (lambda *a: 25 * Func331(*a, **{
'sid': 3016 })))
            if not cl_evcon.CheckPerformType(oTarget, oEventCB, PF_TYPE_CAREERPF, None):
                if cl_evcon.CheckTalentLevel(oTarget, oEventCB, 3016) >= 3:
