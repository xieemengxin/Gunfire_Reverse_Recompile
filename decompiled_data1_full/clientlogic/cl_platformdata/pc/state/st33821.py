# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/state/st33821.pyc
# RelativePath: clientlogic/cl_platformdata/pc/state/st33821.pyc
# Source Generated with Decompyle++
# File: st33821.pyc (Python 3.6)
...
def CallBack0(oEventCB, oTarget):
    ...
    if cl_condition.CheckHero(oTarget, oEventCB.GetCBLifeCycle(), 212):
        cl_action.CommonUsePerform(oTarget, oEventCB.GetCBLifeCycle(), 8013, {
            'vEnd': oEventCB.GetCBLifeCycle().m_Owner.GetArgValue('TargetPos'),
            'DirectPos': 1 })
    elif cl_condition.CheckHero(oTarget, oEventCB.GetCBLifeCycle(), 217):
        cl_action.CommonUsePerform(oTarget, oEventCB.GetCBLifeCycle(), 8009, {
            'vEnd': oEventCB.GetCBLifeCycle().m_Owner.GetArgValue('TargetPos'),
            'DirectPos': 1 })
    elif cl_condition.CheckHero(oTarget, oEventCB.GetCBLifeCycle(), 215):
        cl_action.CommonUsePerform(oTarget, oEventCB.GetCBLifeCycle(), 8007, {
            'vEnd': oEventCB.GetCBLifeCycle().m_Owner.GetArgValue('TargetPos'),
            'DirectPos': 1 })
