# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/clientactive/p12033.pyc
# RelativePath: clientlogic/cl_platformdata/pc/clientactive/p12033.pyc
# Source Generated with Decompyle++
# File: p12033.pyc (Python 3.6)
...
class CCartoon6(TraceCartoon):
    ...
    def Hit(cls, skill):
        cl_action.PerformDamage(skill, {
            'Att': (cl_action.GetDiceAbilityQuality(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_INT)) * 100 + 100 + (150 if cl_action.GetDiceAbilityQuality(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_INT)) > 3 else 100) * cl_action.GetAllTalentLevelSum(skill)) * 100 })
...
class CCartoon0(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cartoon = { }
        CCartoon6.Init(skill, cartoon, casting = 1, index = cl_action.GetCartoonLoopID(skill, 0))
    ...
    if cl_action.IsHeroCtrl(skill):
        ...
        if len(cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)) > 0:
            ...
            for i2 in range(1, cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY), 1):
                cartoon = { }
                CCartoon0.Init(skill, cartoon, casting = 1, index = i2)
