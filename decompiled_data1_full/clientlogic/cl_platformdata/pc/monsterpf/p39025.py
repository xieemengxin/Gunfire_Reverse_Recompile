# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39025.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39025.pyc
...
class CCartoon1(SectorDiffuseCartoon):
    ...
    def Hit(cls, skill):
        if not cl_action.CheckHasSkillVarCache(skill, cl_action.IntToString(skill, cl_action.GetCurVID(skill))):
            ...
            if cl_action.CheckVictimType(skill, WARRIOR_BUILD, OBJ_ALL) or cl_action.CheckVictimSID(skill, 1185):
                cl_action.ChangeVictimDefValue(skill, cl_action.ToInt(skill, cl_action.GetSkillVictimAttr(skill, 'HPMax') * cl_action.GetPerformArgValue(skill, 'buildatk', iDefault = 50) * -0.01), DAM_USE_HP)
            else:
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_math.Vec3Add(cl_action.GetEndPositionInCrt(skill, 3), (0, 0.2, 0)), cl_action.GetSkillVarCache(skill, 'EndPos'), 0, 60, 1.2, 30, 0.5, 20, targettype = OBJ_ENEMY, isOverByFightType = True, fightType = WARRIOR_LUOHOU_PILLAR, isUseSector = True)
...
class CCartoon3(DirectPosCartoon):
    ...
    def End(cls, skill):
        cl_action.SetSkillVarCache(skill, 'EndPosList', cl_action.CreatePosOnCircle(skill, cl_math.Vec3Add(cl_action.GetEndPositionInCrt(skill, 3), (0, 0.2, 0)), 10, cl_action.ToInt(skill, 18)))
