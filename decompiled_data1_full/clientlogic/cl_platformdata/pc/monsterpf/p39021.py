# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39021.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39021.pyc
...
class CCartoon3(SectorRotateCartoon):
    ...
    def Hit(cls, skill):
        if cl_action.CheckVictimType(skill, WARRIOR_BUILD, OBJ_ALL):
            if cl_action.CheckVictimSID(skill, 1184):
                ...
                cl_action.PushHeroVictim(skill, (0, 0, 0), 20, 10, 10000, downSpeed = 10, fGravaty = 9.8, angle = 60, bDirectRotation = True)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgSelfPos(skill), cl_action.GetSkillVarCache(skill, 'WaveDir'), (0, 8, 0), 90, (60 if cl_action.GetMonsterPhase(skill) >= 3 else 45) * ((cl_action.GetAttackerStateCount(skill, 8111, dState = { }) + 100) / 100), 11, 50, targettype = OBJ_ENEMY, innerRadius = 30)
...
class CCartoon2(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, int(160 if cl_action.GetMonsterPhase(skill) >= 2 else 240), 1)
...
def TryAdjustWaveDir(oSkill, *args):
    ...
    if lstSealWallID:
        ...
        if cl_math.IsZero(vDir):
            WarobjLog.Debug('%s AID:%s VID:%s APos:%s VPos:%s' % (oGame.m_ID, oAttack.m_ID, oVictim.m_ID, oAttack.GetPos(), vVictimPos))
            vDir = (1, 1, 1)
