# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39220.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39220.pyc
# Source Generated with Decompyle++
# File: p39220.pyc (Python 3.6)
...
from cl_commondefines import ATT_SHAPE_RECTANGLE, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, OBJ_ENEMY, OBJ_FRIEND
...
class CCartoon5(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 115), 1)
...
class CCartoon3(DirectPosCartoon):
    ...
    def Hit(cls, skill):
        ...
        cl_action.PushHeroVictim(skill, (0, 0, 0), 20, 3.5, 10000, downSpeed = 5, fGravaty = 9.8, angle = 70, bDirectRotation = False)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgDestPosDirPlane(skill, cl_action.CrtArgSelfPos(skill), cl_action.GetSkillCustomData(skill, 'vDir', defaultValue = (0, 0, 0)), cl_action.GetTimerCartoonCurTimes(skill, 4) * 10, 0), cl_action.CrtArgDestPosDirPlane(skill, cl_action.CrtArgSelfPos(skill), cl_action.GetSkillCustomData(skill, 'vDir', defaultValue = (0, 0, 0)), 100, 0), [
                10,
                10,
                7], attshape = ATT_SHAPE_RECTANGLE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE, multipleExplode = True, explosionDelay = 10)
...
class CCartoon0(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.AttackerAddState(skill, 7977, 100, 0, { })

    Trigger = classmethod(Trigger)
    
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 120), 1)
...
class CCartoon7(TimerCartoon):
    ...
    def Active(cls, skill):
        cl_action.MonsterAttackerFacePos(skill, cl_action.CrtArgDestPosDirPlane(skill, cl_action.CrtArgSelfPos(skill), cl_action.GetSkillCustomData(skill, 'vDir', defaultValue = (0, 0, 0)), 100, 0), 180)
        cartoon = { }
