# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39091.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39091.pyc
...
class CCartoon6(EntityCurveCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (0, 0, 0), cl_action.CrtArgTargetPos(skill, notContainDying = False), 0.8, 1033, 180, 16 if cl_action.GetPlayRound(skill) >= 3 else 15 if cl_action.GetPlayRound(skill) >= 2 else 14, 60, 0.3, targettype = OBJ_ENEMY, summonID = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[cl_action.GetCartoonLoopID(skill, 0)], forceDel = True, iPyFlag = PY_FLAG_DEAD)
...
class CCartoon2(TimerCartoon):
    ...
    def Trigger(cls, skill):
        ...
        cl_action.SendCurCartoonTriggerMsg(skill, dInfo = {
            'StoneNum': cl_action.ToInt(skill, (12 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 4 else 10 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 3 else 8 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 2 else 6) * cl_action.GetSkillCacheExtraTrajectory(skill) / cl_action.GetPerformArgValue(skill, 'ChangeStoneNumFactor', iDefault = 1)) })
        cl_action.CreateRandomNumWarriorAtPointPos(skill, cl_action.CreateBoxPosList(skill, cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), (-5, 24, -20)), (12, 24, 12), cl_action.ToInt(skill, (12 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 4 else 10 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 3 else 8 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 2 else 6) * cl_action.GetSkillCacheExtraTrajectory(skill) / cl_action.GetPerformArgValue(skill, 'ChangeStoneNumFactor', iDefault = 1)), 1, 0.8), WARRIOR_SUMMON, {
            1033: 10 }, {
            'Radius': 1 })
