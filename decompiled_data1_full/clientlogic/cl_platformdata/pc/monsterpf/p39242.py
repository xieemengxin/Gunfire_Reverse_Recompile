# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39242.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39242.pyc
# Source Generated with Decompyle++
# File: p39242.pyc (Python 3.6)
...
from cl_commondefines import ATT_SHAPE_SECTOR, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, FIGHT3_KEY_IGNOREIMMOBILIZE, OBJ_ENEMY, SKILLCACHE_INT
...
class CCartoon7(TimerCartoon):
    ...
    def Active(cls, skill):
        cl_action.StartBackSwing(skill, cl_action.ToInt(skill, 80 / (cl_action.GetPlayRound(skill) * 0.2 + 0.8)))
        cartoon = { }
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgSelfPos(skill), cl_action.CrtArgSkillEndPos(skill), [
                12,
                3,
                180], attshape = ATT_SHAPE_SECTOR, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE, multipleExplode = True, explosionDelay = 10)
...
class CCartoon6(TimerCartoon):
    ...
    def Active(cls, skill):
        cl_action.StartBackSwing(skill, cl_action.ToInt(skill, 135 / (cl_action.GetPlayRound(skill) * 0.2 + 0.8)))
        cartoon = { }
...
def Action(skill):
    if cl_action.GetAttackSID(skill) == 39251:
        ...
        cl_action.SetSkillCacheData(skill, SKILLCACHE_INT, cl_action.CrtArgRandomNum(skill, 0, 100))
        cl_action.ServerSendSkillCache(skill, [
            SKILLCACHE_INT])
        if cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) > 50:
            cartoon = { }
...
from cl_commondefines import CLOSE_DISTANCE, DAM_TYPE_NORMAL, MONSTERPF_TYPE_OTHER
