# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39024.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39024.pyc
# Source Generated with Decompyle++
# File: p39024.pyc (Python 3.6)

from cl_only import SendAlert, OutputPos, ShufferList
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgDestPosDir(skill, cl_action.CrtArgSelfPos(skill), cl_action.CrtArgSelfFace(skill), 25, 0), (0, 0, 0), [
                10], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = True, explosion = False, extCheck = CRT_EXTCHECK_NONE, multipleExplode = True, explosionDelay = 10)
...
def UpdateFinalWarnPos(oSkill, *args):
    ...
    iObstacleSID = args[0][3]
    iMaxCnt = args[0][4]
    iNowCnt = cl_action.GetSceneObstacleNumberBySID(oSkill, iObstacleSID)
    iCnt = iMaxCnt - iNowCnt
    if iCnt <= 0:
        return None
    oAttack = oSkill.GetAttack()
    ...
    if len(lstHero) > iCnt:
        lstHero = ShufferList(oGame, lstHero, iCnt)
