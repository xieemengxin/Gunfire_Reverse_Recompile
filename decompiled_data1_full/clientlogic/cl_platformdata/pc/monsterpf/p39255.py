# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39255.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39255.pyc
# Source Generated with Decompyle++
# File: p39255.pyc (Python 3.6)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetEndPositionInCrt(skill, 0), (0, 0, 0), [
                4], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, isCheckServant = False)
...
def Action(skill):
    cl_action.SetSkillVarCache(skill, 'randomstart', [
        ...
        (1740.2, 578.7, 1252.9),
        (1732.8, 575.3, 1218)])
    cl_action.SetSkillCacheData(skill, SKILLCACHE_INT, cl_action.GetSkillCustomData(skill, 'random'))
    cl_action.ServerSendSkillCache(skill, [
        SKILLCACHE_INT])
    if 0 == cl_action.GetSkillCacheData(skill, SKILLCACHE_INT):
        cl_action.SetSkillVarCache(skill, 'lsthero', cl_action.GetLiveHeroID(skill, False, fMaxDis = 0))
        for i1 in range(0, len(cl_action.GetSkillVarCache(skill, 'lsthero')), 1):
            ...
            cl_action.AddClientEffect(skill, 1023, cl_action.CrtArgWarriorPos(skill, cl_action.GetSkillVarCache(skill, 'lsthero')[i1]), 500, (0, 0, 0))
            cartoon = { }
            CCartoon0.Init(skill, cartoon, casting = 0, index = i1)
        
    else:
        cl_action.SetSkillVarCache(skill, 'lstpos', cl_action.GetListByShuffleAndNumber(skill, cl_action.CreateRectanglePosList(skill, (1610.38, 500.31, 1263.58), (60, 0, 60), 36, 4, 0), 2))
        for i2 in range(0, len(cl_action.GetSkillVarCache(skill, 'lstpos')), 1):
            ...
            cl_action.AddClientEffect(skill, 1023, cl_action.GetSkillVarCache(skill, 'lstpos')[i2], 500, (0, 0, 0))
            cartoon = { }
