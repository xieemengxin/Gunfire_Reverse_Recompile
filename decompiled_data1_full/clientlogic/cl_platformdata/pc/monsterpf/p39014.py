# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39014.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39014.pyc
...
from cl_commondefines import CRT_CHECK_SERVER, FIGHT3_KEY_IGNOREKNOCKBACK, FIGHT3_KEY_IGNORETHUMP, FIGHT3_KEY_IGNOREUNBALANCE, SKILLCACHE_EXTRATRAJECTORY, SKILLCACHE_INT, STATE_EFF_DEBAR
...
class CCartoon9(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.SummonAreaMonster(skill, 2, {
            ...
            (8 if cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) >= 4 else (cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True)) * 2) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) + 1 + (2 if cl_action.GetPlayRound(skill) >= 3 else 1 if cl_action.GetPlayRound(skill) >= 2 else 0): 100,
            (8 if cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) >= 4 else (cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True)) * 2) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) + (2 if cl_action.GetPlayRound(skill) >= 3 else 1 if cl_action.GetPlayRound(skill) >= 2 else 0): 100 })
...
class CCartoon3(MonsterJumpCartoon):
    ...
    def End(cls, skill):
        ...
        cl_action.SetForceNoDestFlag(skill, False)
...
class CCartoon0(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.SummonAreaMonster(skill, 2, {
            ...
            (8 if cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) >= 4 else (cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True)) * 2) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) + 1 + (2 if cl_action.GetPlayRound(skill) >= 3 else 1 if cl_action.GetPlayRound(skill) >= 2 else 0): 100,
            (8 if cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) >= 4 else (cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True)) * 2) + cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) + (2 if cl_action.GetPlayRound(skill) >= 3 else 1 if cl_action.GetPlayRound(skill) >= 2 else 0): 100 })
        if cl_action.GetSkillCacheData(skill, SKILLCACHE_EXTRATRAJECTORY) + 1 >= 4:
            cartoon = { }
            CCartoon6.Init(skill, cartoon, casting = 1, index = 0)
        else:
            cl_action.AttackerAddState(skill, 7014, 710 + cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) * 300 if cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) > 0 else 710 - cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) * 100, 0, { })
            cartoon = { }
...
class CCartoon2(MonsterJumpCartoon):
    ...
    def End(cls, skill):
        cl_action.SetAttackerCustomIntData(skill, 'PF39014_Stone', 1)
        cl_action.RemoveIgnoreStateEffect(skill, STATE_EFF_DEBAR)
        cartoon = { }
...
def Action(skill):
    ...
    cl_action.SetForceNoDestFlag(skill, True)
    cl_action.SetSkillCacheData(skill, SKILLCACHE_INT, cl_action.GetPerformArgValue(skill, 'Extrasummon', iDefault = 0))
    cl_action.SetSkillCacheExtraTrajectory(skill, cl_action.GetMonsterPerformRecord(skill, 39014))
