# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39096.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39096.pyc
# Source Generated with Decompyle++
# File: p39096.pyc (Python 3.6)
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, OBJ_ENEMY, SKILLCACHE_EXTRATRAJECTORY, SKILLCACHE_INT, SKILLCACHE_LIVEHEROCNT, SKILLCACHE_LSTINT, WARRIOR_SUMMON
from cl_only import PY_FLAG_DEAD, PY_FLAG_EXCLUDEMONSTERHATE
...
class CCartoon6(EntityCurveCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (0, 0, 0), cl_action.CrtArgTargetPos(skill, notContainDying = False), 0.8, 1033, 180, 17 if cl_action.GetPlayRound(skill) >= 3 else 16 if cl_action.GetPlayRound(skill) >= 2 else 15, 100, 0.3, targettype = OBJ_ENEMY, summonID = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[cl_action.GetCartoonLoopID(skill, 0)], forceDel = True, iPyFlag = PY_FLAG_DEAD)
...
class CCartoon5(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, 100 if cl_action.GetPlayRound(skill) >= 3 else 125 if cl_action.GetPlayRound(skill) >= 2 else 150, cl_action.GetSkillCacheData(skill, SKILLCACHE_INT))
...
class CCartoon2(TimerCartoon):
    ...
    def Trigger(cls, skill):
        ...
        cl_action.SendCurCartoonTriggerMsg(skill, dInfo = {
            'StoneNum': cl_action.ToInt(skill, ((12 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 4 else 10 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 3 else 8 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 2 else 6) + 2) * cl_action.GetSkillCacheExtraTrajectory(skill) / cl_action.GetPerformArgValue(skill, 'ChangeStoneNumFactor', iDefault = 1)) })
        cl_action.CreateRandomNumWarriorAtPointPos(skill, cl_action.CreateBoxPosList(skill, cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), (-5, 24, -20)), (12, 24, 12), cl_action.ToInt(skill, ((12 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 4 else 10 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 3 else 8 if cl_action.GetAllHeroCnt(skill, True, bUseRidingAloneCnt = False) >= 2 else 6) + 2) * cl_action.GetSkillCacheExtraTrajectory(skill) / cl_action.GetPerformArgValue(skill, 'ChangeStoneNumFactor', iDefault = 1)), 1, 0.8), WARRIOR_SUMMON, {
            1033: 10 }, {
            'Radius': 1 })
