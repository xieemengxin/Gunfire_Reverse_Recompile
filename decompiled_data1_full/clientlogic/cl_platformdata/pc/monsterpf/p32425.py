# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p32425.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p32425.pyc
...
class CCartoon5(DelegateThrowCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (1.5, 0.8, 1.3)), cl_action.CrtArgSetParabolaSpeed(skill, cartoon, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (1.5, 0.8, 1.3)), cl_action.CrtArgTargetPos(skill, notContainDying = False), 18, -18, 0.5), 5, 1, (0, 0, 0), (1, 1), 800, True, liveTime = 0, innerRadius = 0.3, pierce = 1)
...
class CCartoon4(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_math.Vec3Add(cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), cl_action.CrtArgDestPosDirPlane(skill, (0, 0, 0), cl_action.CrtArgSelfFace(skill), 2, cl_action.ToInt(skill, cl_action.ToInt(skill, cl_action.GetTimerCartoonCurTimes(skill, 3) / 2) * -180 + 90))), (0, 3, 0)), cl_math.Vec3Add(cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), cl_action.CrtArgDestPosDirPlane(skill, (0, 0, 0), cl_action.CrtArgSelfFace(skill), 2, cl_action.ToInt(skill, cl_action.ToInt(skill, cl_action.GetTimerCartoonCurTimes(skill, 3) / 2) * -180 + 90))), (0, 3, 0)), cl_action.CrtArgTargetPos(skill, notContainDying = False), 1, 30, 30, targettype = OBJ_ENEMY, liveTime = 0, radius = 0.3, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon16(DirectPosCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetEndPositionInCrt(skill, 18), (0, 0, 0), [
                5], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE, multipleExplode = True, explosionDelay = 10)
...
class CCartoon18(RayCastCartoon):
    m_SID = 18
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_math.Vec3Add(cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), cl_action.CrtArgDestPosDirPlane(skill, (0, 0, 0), cl_action.CrtArgSelfFace(skill), 2, cl_action.ToInt(skill, cl_action.ToInt(skill, cl_action.GetTimerCartoonCurTimes(skill, 12) / 2) * -180 + 90))), (0, 3, 0)), cl_math.Vec3Add(cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), cl_action.CrtArgDestPosDirPlane(skill, (0, 0, 0), cl_action.CrtArgSelfFace(skill), 2, cl_action.ToInt(skill, cl_action.ToInt(skill, cl_action.GetTimerCartoonCurTimes(skill, 12) / 2) * -180 + 90))), (0, 3, 0)), cl_action.CrtArgTargetPos(skill, notContainDying = False), 1, 30, 30, targettype = OBJ_ENEMY, liveTime = 0, radius = 0.3, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
...
def Action(skill):
    cl_action.SetSkillCacheData(skill, SKILLCACHE_RANDOM, cl_action.CrtArgRandomNum(skill, 0, 1))
    cl_action.ServerSendSkillCache(skill, [
        SKILLCACHE_RANDOM])
    if cl_action.GetSkillCacheData(skill, SKILLCACHE_RANDOM) == 0:
        cartoon = { }
