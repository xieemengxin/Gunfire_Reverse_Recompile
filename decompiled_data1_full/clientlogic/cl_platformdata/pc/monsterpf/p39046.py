# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39046.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39046.pyc
# Source Generated with Decompyle++
# File: p39046.pyc (Python 3.6)
...
class CCartoon2(TimerCartoon):
    ...
    def Active(cls, skill):
        ...
        if cl_action.GetRandomLivePlayer(skill, 120, bNotContainDying = True, fMaxDis = 62, bSkillVIDSecond = False, iFlag = PY_FLAG_MONSTERTARGET, bResetVID = True) == 0 or cl_action.GetSkillVarCache(skill, 'HasLastHeroPos') == 0:
            cl_action.SetSkillVarCache(skill, 'LaunchEndPosList', cl_action.GetSummonPosByBoxSplit(skill, 10 if cl_action.GetMonsterPhase(skill) >= 3 else 15 if cl_action.GetMonsterPhase(skill) >= 2 else 20, 5, 1, False, {
                cl_action.ToInt(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) - 1): 100 }, { }, True, iAngle = 0))
            ...
            cl_action.SetSkillVarCache(skill, 'HasLastHeroPos', 1)
        else:
            cl_action.SetSkillVictim(skill, cl_action.GetRandomLivePlayer(skill, 120, bNotContainDying = True, fMaxDis = 62, bSkillVIDSecond = False, iFlag = PY_FLAG_MONSTERTARGET, bResetVID = True))
            cl_action.SetSkillVarCache(skill, 'LaunchEndPosList', cl_action.GetSummonPosByBoxSplit(skill, 10 if cl_action.GetMonsterPhase(skill) >= 3 else 15 if cl_action.GetMonsterPhase(skill) >= 2 else 20, 5, 1, False, {
                cl_action.ToInt(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) - 1): 100 }, { }, True, iAngle = 0))
            cl_action.GetSkillVarCache(skill, 'LaunchEndPosList').append(cl_action.CrtArgTargetPos(skill, notContainDying = True))
            cl_action.SetSkillVarCache(skill, 'LaunchEndPosList', cl_action.ChangePosListToCloseGround(skill, cl_action.GetSkillVarCache(skill, 'LaunchEndPosList')))
...
def Action(skill):
    cl_action.SetSkillCacheData(skill, SKILLCACHE_CHARGELEVEL, cl_action.GetPerformArgValue(skill, 'TrajectoryCnt', iDefault = 1))
    ...
    cl_action.SetSkillVarCache(skill, 'HasLastHeroPos', 0)
