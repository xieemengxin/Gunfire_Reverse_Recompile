# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p20411.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p20411.pyc
# Source Generated with Decompyle++
# File: p20411.pyc (Python 3.6)
...
class CCartoon1(DelegateCurveCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 1.2, -0.5)), cl_action.CrtArgRandomAngle(skill, cartoon, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 1.2, -0.5)), cl_action.CrtArgTargetBodyShiftPos(skill, 0, 0, 5, 15, 0, 0, 0, 0), 0, 0, 0, 0, False), 1, 105, 35, 0, 0.4, targettype = OBJ_ENEMY, effect = 0, liveTime = 0, trailEffect = 0, effectLiveTime = 0, defLockPos = (0, 0, 0), weakdis = 0, weakangle = 0, resetTargetPos = False)

    InitSuccess = classmethod(InitSuccess)


class CCartoon2(DelegateCurveCartoon):
    m_SID = 2
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 1, -1.4)), cl_action.CrtArgRandomAngle(skill, cartoon, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 1.2, -0.5)), cl_action.CrtArgTargetBodyShiftPos(skill, 0, 0, 5, 15, 0, 0, 0, 0), 0, 0, 0, 0, False), 1, 105, 35, 0, 0.4, targettype = OBJ_ENEMY, effect = 0, liveTime = 0, trailEffect = 0, effectLiveTime = 0, defLockPos = (0, 0, 0), weakdis = 0, weakangle = 0, resetTargetPos = False)
...
class CCartoon0(TimerCartoon):
    ...
    def Trigger(cls, skill):
        if cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) > 0:
            ...
            cl_action.SetSkillCacheData(skill, SKILLCACHE_LSTINT, cl_action.GetHeroInSightArgs(skill, 60, 45, True, cl_action.GetSkillVarCache(skill, 'MainTarget'), cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 1, -1.4))))
