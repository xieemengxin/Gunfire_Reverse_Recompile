# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p21811.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p21811.pyc
...
from cl_commondefines import ATT_SHAPE_SECTOR, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, OBJ_ALLNOSELF, OBJ_ENEMY, SKILLCACHE_EXTRATRAJECTORY, WARRIOR_HERO, WARRIOR_MONSTER, WARRIOR_SUMMON
...
class CCartoon4(EntityParabolaCartoon):
    ...
    def Hit(cls, skill):
        if cl_action.CheckVictimType(skill, WARRIOR_MONSTER, OBJ_ALLNOSELF):
            cl_action.PushVictim(skill, cl_action.GetCartoonCurPos(skill, 4), 10, 3, 10000, angle = 315, iCartoonSID = -1, iIgnoreStruckCD = 0, iClient = 0)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (0, 0, 0), cl_math.Vec3Minus(cl_action.CrtArgSelfCenterPos(skill), cl_action.GetSceneObjPos(skill, cl_action.GetSkillSummonCreate(skill)[0])), 50, 0.4, (0, 18, 0), (0, 0), (0, 0), 500, False, False, 0, False, cl_action.GetSkillSummonCreate(skill)[0], 0, targetType = OBJ_ALLNOSELF, maxDistance = cl_math.CalDistance3D(cl_action.GetSceneObjPos(skill, cl_action.GetSkillSummonCreate(skill)[0]), cl_action.GetMonsterMuzzlePos(skill, (0, 1, -2.5))) + 0.01, hitHeroOver = False, forceSpeed = True, innerRadius = 0.1, scale = 1, hitmonsterover = False)
...
class CCartoon2(DirectPosCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgSelfPos(skill), cl_action.GetEndPositionInCrt(skill, 5), [
                6.5,
                3,
                120], attshape = ATT_SHAPE_SECTOR, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE, multipleExplode = True, explosionDelay = 10)
...
class CCartoon0(TimerCartoon):
    ...
    def Trigger(cls, skill):
        ...
        if cl_action.GetSkillVarCache(skill, 'pushoverflag') == 2:
            ...
            if cl_math.CalDistance3D(cl_action.CrtArgSelfPos(skill), cl_action.GetEndPositionInCrt(skill, 5)) > 6.5:
                cartoon = { }
...
class CCartoon5(MonsterPushHeroCartoon):
    ...
    def Hit(cls, skill):
        if cl_action.CheckVictimType(skill, WARRIOR_MONSTER, OBJ_ALLNOSELF):
            cl_action.PushVictim(skill, cl_action.GetCartoonCurPos(skill, 5), 10, 3, 10000, angle = 315, iCartoonSID = -1, iIgnoreStruckCD = 0, iClient = 0)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (cl_math.CalDistance3D(cl_action.GetSceneObjPos(skill, cl_action.GetSkillSummonCreate(skill)[0]), cl_action.GetMonsterMuzzlePos(skill, (0, 1, -2.5))) + 0.3) / 0.4, cl_math.Vec3MulV(cl_math.Vec3Minus(cl_action.CrtArgSelfCenterPos(skill), cl_action.GetSceneObjPos(skill, cl_action.GetSkillSummonCreate(skill)[0])), (1, 0, 1)), cl_math.CalDistance3D(cl_action.GetSceneObjPos(skill, cl_action.GetSkillSummonCreate(skill)[0]), cl_action.GetMonsterMuzzlePos(skill, (0, 1, -2.5))) + 0.3 if cl_math.CalDistance3D(cl_action.CrtArgSelfPos(skill), cl_action.GetSceneObjPos(skill, cl_action.GetSkillVarCache(skill, 'pushhero'))) > 2.6 else 0, attachSummon = cl_action.GetSkillSummonCreate(skill)[0], targetType = OBJ_ALLNOSELF, lockTarget = cl_action.GetSkillVarCache(skill, 'pushhero'))
...
class CCartoon3(EntityParabolaCartoon):
    ...
    def Hit(cls, skill):
        if cl_action.CheckVictimType(skill, WARRIOR_MONSTER, OBJ_ALLNOSELF):
            cl_action.PushVictim(skill, cl_action.GetCartoonCurPos(skill, 3), 10, 3, 10000, angle = 45, iCartoonSID = -1, iIgnoreStruckCD = 0, iClient = 0)
        elif cl_action.CheckVictimType(skill, WARRIOR_HERO, OBJ_ALLNOSELF):
            if cl_action.CheckPopupHook(skill, {
                'State': 32774,
                'CosAngle': 75 }):
                cl_action.WeaponDamage(skill, {
...
def Halt(skill):
    cl_action.AttackerUsePerform(skill, 21812, {
        'summoncreate': cl_action.GetSkillSummonCreate(skill)[0] if len(cl_action.GetSkillSummonCreate(skill)) > 0 else 0 }, 4, 0)
