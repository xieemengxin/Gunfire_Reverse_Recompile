# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p21651.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p21651.pyc
# Source Generated with Decompyle++
# File: p21651.pyc (Python 3.6)
...
class CCartoon0(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (-0.191, 0.97, -1.719)), cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (-0.191, 0.97, -1.719)), cl_action.GetEndPositionInCrt(skill, 9), 1, 60, 80, targettype = OBJ_ENEMY, liveTime = 0, radius = 0, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon12(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, int(cl_action.GetAttackerAttr(skill, 'AttSpeed') * 2.5), cl_action.ToInt(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_BALLISTICTYPE) + -1))
...
class CCartoon11(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetAttackerAttr(skill, 'AttSpeed'), 1)
