# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p31642.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p31642.pyc
# Source Generated with Decompyle++
# File: p31642.pyc (Python 3.6)
...
from cl_commondefines import ATT_SHAPE_LINE, ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, OBJ_ALL, OBJ_ENEMY, SKILLCACHE_BALLISTICTYPE
...
class CCartoon0(RayCastCartoon):
    ...
    def End(cls, skill):
        cl_action.StartBackSwing(skill, 25)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0.2, 3.67, -2.97)), cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0.2, 3.67, -2.97)), cl_action.GetEndPositionInCrt(skill, 5), 1, 60, 50, targettype = OBJ_ENEMY, effect = 0, liveTime = 0, trailEffect = 0, effectLiveTime = 5, radius = 0, flyoverdis = 0)
...
class CCartoon5(DirectPosCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0.2, 3.67, -2.97)), cl_action.CrtArgTargetPos(skill, notContainDying = False), [
                0], attshape = ATT_SHAPE_LINE, targettype = OBJ_ALL, pierceStatic = False, explosion = False)
...
class CCartoon19(DirectPosCartoon):
    m_SID = 19
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetEndPositionInCrt(skill, 9), (0, 0, 0), [
...
class CCartoon18(DirectPosCartoon):
    m_SID = 18
    
    def Active(cls, skill):
        cartoon = { }
        CCartoon17.Init(skill, cartoon, casting = 0, index = 0)
...
class CCartoon9(RayCastCartoon):
    m_SID = 9
    ...
    def Hit(cls, skill):
        cartoon = { }
        CCartoon18.Init(skill, cartoon, casting = 0, index = 0)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0.2, 3.67, -2.97)), cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0.2, 3.67, -2.97)), cl_action.GetEndPositionInCrt(skill, 16), 1, 60, 50, targettype = OBJ_ENEMY, effect = 0, liveTime = 0, trailEffect = 0, effectLiveTime = 5, radius = 0, flyoverdis = 0)
...
class CCartoon7(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetAttackerAttr(skill, 'AttSpeed'), cl_action.ToInt(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_BALLISTICTYPE) + -2) if cl_action.ToInt(skill, cl_action.GetSkillCacheData(skill, SKILLCACHE_BALLISTICTYPE) + -2) > 0 else 1)
