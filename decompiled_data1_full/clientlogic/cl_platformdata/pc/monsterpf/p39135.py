# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/monsterpf/p39135.pyc
# RelativePath: clientlogic/cl_platformdata/pc/monsterpf/p39135.pyc
...
class CCartoon1(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, (35 if cl_action.GetPlayRound(skill) < 3 else cl_action.GetSkillCacheData(skill, SKILLCACHE_INT)) - cl_action.GetPlayRound(skill) * 2), 13 if cl_action.GetPlayRound(skill) >= 3 else 11 if cl_action.GetPlayRound(skill) >= 2 else 10)
...
def Action(skill):
    cl_action.SetSkillCacheData(skill, SKILLCACHE_INT, cl_action.GetPerformArgValue(skill, 'Speed'))
