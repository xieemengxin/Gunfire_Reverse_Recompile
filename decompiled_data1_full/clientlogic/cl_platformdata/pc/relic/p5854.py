# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/relic/p5854.pyc
# RelativePath: clientlogic/cl_platformdata/pc/relic/p5854.pyc
# Source Generated with Decompyle++
# File: p5854.pyc (Python 3.6)
...
def DoCallBackAction0(oEventCB, oWarrior):
    if cl_evcon.CheckEventWeaponBulletType(oWarrior, oEventCB, 4504):
        cl_evact.EventGetTargetByType(oWarrior, oEventCB, OBJ_ATTACK)
        if cl_evcon.GetTargetBagBulletAmount(oWarrior, oEventCB, 4503) >= cl_condition.CalFormula(oWarrior, oEventCB.GetCBLifeCycle(), cl_evcon.GetBulletUseByType(oWarrior, oEventCB, 4504) * 4):
            cl_evact.EventCBCostBagBulletByType(oWarrior, oEventCB, 4503, cl_evcon.GetBulletUseByType(oWarrior, oEventCB, 4504) * 4)


def DoCallBackAction2(oEventCB, oWarrior):
    if cl_evcon.CheckEventWeaponBulletType(oWarrior, oEventCB, 4504):
        cl_evact.EventCBChangeLuckyHit(oWarrior, oEventCB, (lambda *a: Func648(*a) * 12))
...
def DoCallBackAction5(oEventCB, oWarrior):
    if cl_evcon.CheckEventWeaponBulletType(oWarrior, oEventCB, 4504):
        cl_evact.EventCBChangeLuckyHit(oWarrior, oEventCB, cl_evcon.GetBulletUseByType(oWarrior, oEventCB, 4503) * 12)
