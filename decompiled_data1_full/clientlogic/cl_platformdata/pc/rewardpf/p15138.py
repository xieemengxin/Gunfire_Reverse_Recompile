# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/pc/rewardpf/p15138.pyc
# RelativePath: clientlogic/cl_platformdata/pc/rewardpf/p15138.pyc
# Source Generated with Decompyle++
# File: p15138.pyc (Python 3.6)
...
from cl_commondefines import DAM_TYPE_CORRISION, DAM_TYPE_NORMAL, MAIN_HOLD, OBJECT_OWNER, OBJECT_SERVANT, PF_SUBMSG_CAREERPF, PF_SUBMSG_THROW, PF_TYPE_CAREERPF, PF_TYPE_THROW, WEAPON_ELEMENTREFRESH_REMOVE

def Action1(oWarrior, oLifeCycle):
    cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oLifeCycle, PF_TYPE_THROW, OBJECT_OWNER, DAM_TYPE_CORRISION, 1, 1)
    ...
    if cl_condition.CheckHero(oWarrior, oLifeCycle, 217):
        cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oLifeCycle, PF_TYPE_CAREERPF, OBJECT_SERVANT, DAM_TYPE_CORRISION, 1, 1)
        cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oLifeCycle, PF_TYPE_THROW, OBJECT_SERVANT, DAM_TYPE_CORRISION, 1, 1)
    ...
    if not cl_condition.CheckHero(oWarrior, oLifeCycle, 201) or cl_condition.CheckHero(oWarrior, oLifeCycle, 207) or cl_condition.CheckHero(oWarrior, oLifeCycle, 215) or cl_condition.CheckHero(oWarrior, oLifeCycle, 221):
        cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oLifeCycle, PF_TYPE_CAREERPF, OBJECT_OWNER, DAM_TYPE_CORRISION, 1, 1)
...
def DoCallBackAction1(oEventCB, oWarrior):
    if cl_evcon.EventCBPerformDamType(oWarrior, oEventCB, OBJECT_OWNER, DAM_TYPE_NORMAL, 1):
        cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oEventCB.GetCBLifeCycle(), PF_TYPE_CAREERPF, OBJECT_OWNER, DAM_TYPE_CORRISION, 1, 1)
        cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oEventCB.GetCBLifeCycle(), PF_TYPE_THROW, OBJECT_OWNER, DAM_TYPE_CORRISION, 1, 1)
    else:
...
def DoCallBackAction2(oEventCB, oWarrior):
    if cl_evcon.EventCBPerformDamType(oWarrior, oEventCB, OBJECT_SERVANT, DAM_TYPE_NORMAL, 1):
        cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oEventCB.GetCBLifeCycle(), PF_TYPE_CAREERPF, OBJECT_SERVANT, DAM_TYPE_CORRISION, 1, 1)
        cl_action.CommonChangePerformDamTypeByPerformType(oWarrior, oEventCB.GetCBLifeCycle(), PF_TYPE_THROW, OBJECT_SERVANT, DAM_TYPE_CORRISION, 1, 1)
    else:
        cl_evact.EventCBClearPerformDamType(oWarrior, oEventCB, OBJECT_SERVANT)


def DoCallBackAction3(oEventCB, oWarrior):
    if cl_evcon.EventCBCheckWeaponElementType(oWarrior, oEventCB, 'ST33315', MAIN_HOLD, DAM_TYPE_NORMAL) and cl_evcon.CheckWeaponElementTypeByHoldType(oWarrior, oEventCB, MAIN_HOLD, DAM_TYPE_NORMAL):
        cl_evact.PassiveCBAddState(oWarrior, oEventCB, 33315, 0, { }, 1, 0, 0)
