# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/pfai/pfai39093.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/pfai/pfai39093.pyc
# Source Generated with Decompyle++
# File: pfai39093.pyc (Python 3.6)
...
def Condition39095(oOwner, dInfo):
    if not oOwner.Phase() == 2 or cl_condition.GetPerformRecord(oOwner, dInfo, {
        ...
        if not oOwner.Phase() == 3 or cl_condition.GetPerformRecord(oOwner, dInfo, {
            39093: 1 }) >= 2 or cl_condition.GetPerformRecord(oOwner, dInfo, {
            39092: 1 }) >= 1:
...
def Condition39095_3_10(oOwner, dInfo):
    if not oOwner.Phase() == 2 or cl_condition.GetPerformRecord(oOwner, dInfo, {
        39093: 1 }) >= 2:
        if not oOwner.Phase() == 3 or cl_condition.GetTargetTypeOjCnt(oOwner, 'StonePillar', 1134) <= cl_condition.GetNoDieNum(oOwner) + 1:
            if (oOwner.Phase() == 1 or cl_condition.GetPerformRecord(oOwner, dInfo, {
                39093: 1 }) >= 3) and oOwner.Phase() == 4:
                pass
    return cl_condition.GetTargetTypeOjCnt(oOwner, 'StonePillar', 1134) <= cl_condition.GetNoDieNum(oOwner) + 1
