# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/state/st39668.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/state/st39668.pyc
# Source Generated with Decompyle++
# File: st39668.pyc (Python 3.6)
...
import cl_evcon
import cl_state
from cl_commondefines import DAM_TYPE_NORMAL, DAM_TYPE_THUNDER, OBJECT_OWNER, OBJECT_SERVANT, OBJ_SELF, PF_SUBMSG_CAREERPF, PF_SUBMSG_THROW, PF_TYPE_CAREERPF, PF_TYPE_THROW, STATE_ADD_REFRESH, STATE_CLS_HELP, STATE_EFF_NONE
from cl_newformula import Func402

def StateActAction(oTarget, oLifeCycle):
    cl_action.CommonChangePerformDamTypeByPerformType(oTarget, oLifeCycle, PF_TYPE_THROW, OBJECT_OWNER, DAM_TYPE_THUNDER, 1, 1)
    ...
    cl_action.CommonRefreshStateExtraInfo(oTarget, oLifeCycle, {
        'SrcLV': (lambda *a: Func402(*a)) }, 33736)
    if cl_condition.CheckHero(oTarget, oLifeCycle, 217):
        cl_action.CommonChangePerformDamTypeByPerformType(oTarget, oLifeCycle, PF_TYPE_CAREERPF, OBJECT_SERVANT, DAM_TYPE_THUNDER, 1, 1)
        cl_action.CommonChangePerformDamTypeByPerformType(oTarget, oLifeCycle, PF_TYPE_THROW, OBJECT_SERVANT, DAM_TYPE_THUNDER, 1, 1)
    ...
    if not cl_condition.CheckHero(oTarget, oLifeCycle, 201) or cl_condition.CheckHero(oTarget, oLifeCycle, 207) or cl_condition.CheckHero(oTarget, oLifeCycle, 215) or cl_condition.CheckHero(oTarget, oLifeCycle, 221):
        cl_action.CommonChangePerformDamTypeByPerformType(oTarget, oLifeCycle, PF_TYPE_CAREERPF, OBJECT_OWNER, DAM_TYPE_THUNDER, 1, 1)
    ...
    cl_action.CommonRefreshStateExtraInfo(oTarget, oLifeCycle, {
        'SrcLV': oLifeCycle.m_Owner.GetArgValue('StatusEffect') }, 33610)
    if oLifeCycle.m_Owner.GetArgValue('StatusEffect') == 4:
        cl_action.CommonChangePositiveElementFactorByType(oTarget, oLifeCycle, DAM_TYPE_THUNDER, 2500)
