# Path: /Users/caoguangpei/个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/state/st33381.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/state/st33381.pyc
# Source Generated with Decompyle++
# File: st33381.pyc (Python 3.6)
...
def CallBack0(oEventCB, oTarget):
    ...
    cl_evact.EventTargetDamage(oTarget, oEventCB, (lambda *a: Func374(*a) * 0.15), DAM_TYPE_PERFORM | DAM_TYPE_TRUE | DAM_USE_ALL, 0, 1, 1, 0, 0, 1, 0, 0, NORMAL_DAMAGE, None, None)


def CallBack1(oEventCB, oTarget):
    if cl_evcon.EventCBGetMsgInfo(oTarget, oEventCB, 'IsDam'):
        if cl_evcon.EventCBCheckFromPointState(oTarget, oEventCB, 33381):
            cl_evact.StateCBSelfAttackerUsePerform(oTarget, oEventCB, 8604, {
                'Att': (lambda *a: cl_evact.EventCBGetTotalHPChangeByCureRatio(oTarget, oEventCB, 1) * 20 + Func223(*a) * 12500 + 50000),
                'Radius': 8 }, 0)
...
'sKey': 'StoreDamage' })) * 20 + Func223(*a) * 12500 + 50000),
                'Radius': 8 }, 0)
...
def CallBack4(oEventCB, oTarget):
    if cl_evcon.EventCBCheckFromPointState(oTarget, oEventCB, 33381) and cl_evcon.CheckDamFromSelf(oTarget, oEventCB, 0):
