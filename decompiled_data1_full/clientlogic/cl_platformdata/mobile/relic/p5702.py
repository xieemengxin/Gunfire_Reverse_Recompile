# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/relic/p5702.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/relic/p5702.pyc
...
from cl_commondefines import EQUIP_TYPE_CLOSEWEAPON, MAIN_HOLD, OBJ_ATTACK, OBJ_VICTIM, QUALITY_TYPE_HIGH, RELIC_TYPE_NORMAL
...
def DoCallBackAction2(oEventCB, oWarrior):
    if not cl_evcon.CheckFromPointPerform(oWarrior, oEventCB, 1610, 0, None):
        cl_evact.EventGetTargetByType(oWarrior, oEventCB, OBJ_ATTACK)
        if cl_evcon.CheckWeaponTypeByHoldType(oWarrior, oEventCB, MAIN_HOLD, EQUIP_TYPE_CLOSEWEAPON):
            if cl_evcon.CheckTargetDist(oWarrior, oEventCB, 15, None, None):
                cl_evact.EventChangeDamFactor(oWarrior, oEventCB, OBJ_VICTIM, 0, -9000, 0, '')
            else:
                cl_evact.EventChangeDamFactor(oWarrior, oEventCB, OBJ_VICTIM, 0, 9000, 0, '')
        elif cl_evcon.CheckTargetDist(oWarrior, oEventCB, 15, None, None):
            cl_evact.EventChangeDamFactor(oWarrior, oEventCB, OBJ_VICTIM, 0, 9000, 0, '')
        else:
            cl_evact.EventChangeDamFactor(oWarrior, oEventCB, OBJ_VICTIM, 0, -9000, 0, '')
