# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/task/task1006.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/task/task1006.pyc
# Source Generated with Decompyle++
# File: task1006.pyc (Python 3.6)
...
def DisableAction(oWarrior, oLifeCycle):
    if cl_condition.TaskCheckTaskStatus(oWarrior, oLifeCycle, TASK_STATUS_FAIL):
        ...
        cl_action.CommonAddStateCount(oWarrior, oLifeCycle, 32932, 1, None)


def ClearAction(oWarrior, oLifeCycle):
    cl_action.CommonAddStateCount(oWarrior, oLifeCycle, 32932, -1, 0)


def DoCallBackAction0(oEventCB, oWarrior):
    cl_evact.EventGetTargetByType(oWarrior, oEventCB, OBJ_VICTIM)
    if cl_condition.HasState(oWarrior, oEventCB.GetCBLifeCycle(), 32926) == 0:
...
class CTask(CBaseTask):
    m_SID = 1006
    m_Name = '毫发无损（1级）'
    m_Description = '完成3个房间前，在至少3秒未受到伤害的情况下击败或助攻8个敌人（精英怪算3倍进度）'
    m_Quality = 1
    m_RewardPerformSID = 40011
