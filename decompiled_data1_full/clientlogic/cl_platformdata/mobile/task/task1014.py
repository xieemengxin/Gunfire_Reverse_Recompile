# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/task/task1014.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/task/task1014.pyc
# Source Generated with Decompyle++
# File: task1014.pyc (Python 3.6)
...
def DisableAction(oWarrior, oLifeCycle):
    if cl_condition.TaskCheckTaskStatus(oWarrior, oLifeCycle, TASK_STATUS_SUCCESS):
        cl_action.CommonAddSavedData(oWarrior, oLifeCycle, '40069Curse', -5)
...
def DoCallBackAction1(oEventCB, oWarrior):
    if cl_evcon.CheckNPCType(oWarrior, oEventCB, NWARRIOR_NPC_SHOP):
        cl_action.TaskAddSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), '1014NPC', 1)


def DoCallBackAction2(oEventCB, oWarrior):
    if cl_condition.CalFormula(oWarrior, oEventCB.GetCBLifeCycle(), (lambda *a: Func247(*a))) != cl_action.TaskGetSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), '1014Level'):
        cl_action.TaskSetSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), '1014Level', (lambda *a: Func247(*a)))
        cl_action.TaskSetSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), 'Temp', (lambda *a: Func716(*a, **{
'sAttr': '1014NPC' })))
        cl_action.TaskSetSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), '1014NPC', 0)
...
def DoCallBackAction5(oEventCB, oWarrior):
    if not cl_action.TaskGetSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), '1014First'):
        cl_action.TaskSetSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), '1014First', 1)
        if cl_condition.CalFormula(oWarrior, oEventCB.GetCBLifeCycle(), (lambda *a: Func598(*a, **{
'sKey': '40069Curse' }))) < 100:
            cl_action.CommonAddSavedData(oWarrior, oEventCB.GetCBLifeCycle(), '40069Curse', 5)
        cl_action.TaskSetSaveInfo(oWarrior, oEventCB.GetCBLifeCycle(), '1014NPC', 0)
