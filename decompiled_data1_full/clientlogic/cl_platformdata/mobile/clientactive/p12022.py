# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/clientactive/p12022.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/clientactive/p12022.pyc
...
class CCartoon2(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetCartoonStart(skill, 6)):
                return None
...
class CCartoon4(TraceCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetCartoonStart(skill, 6)):
                return None
            cls.EnableShow(skill, 1, 25, 20, targettype = OBJ_ENEMY, liveTime = 0, lineDistance = 0, angle = 25, lockWeakness = False, lockAngle = 0, lockDis = 0, IgnoreDefalutDis = 0, iIgnoreMonsterID = cl_action.GetSkillCustomArg(skill, 'MainTarget'), FilterDie = True, lockFromCartoon = False)
