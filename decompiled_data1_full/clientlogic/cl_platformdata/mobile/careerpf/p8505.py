# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/careerpf/p8505.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/careerpf/p8505.pyc
    ...
    def Hit(cls, skill):
        for i1 in range(0, cl_action.GetSkillVarCache(skill, 'sk8505_trajectory'), 1):
            cl_action.WeaponDamage(skill, {
                'Att': int((cl_action.GetSkillCustomData(skill, 'Att') / 100) * cl_action.GetSkillVarCache(skill, 'sk8505_damageratio')) if cl_action.GetSkillCustomData(skill, 'Att') else cl_action.GetSkillVarCache(skill, 'sk8505_damageratio') }, { }, sendPFMsg = False)
...
class CCartoon1(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, 20, cl_action.GetSkillCustomData(skill, 'chopsTimes') if cl_action.GetSkillCustomData(skill, 'chopsTimes') > 0 else 1)
...
def Action(skill):
    ...
    cl_action.SetSkillCacheData(skill, SKILLCACHE_LSTINTSPECIAL, cl_action.CrtArgRandomIntList(skill, 0, 99, cl_action.GetSkillCustomData(skill, 'chopsTimes') if cl_action.GetSkillCustomData(skill, 'chopsTimes') > 0 else 1))
