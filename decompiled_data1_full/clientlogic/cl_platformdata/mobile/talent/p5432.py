# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/talent/p5432.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/talent/p5432.pyc
# Source Generated with Decompyle++
# File: p5432.pyc (Python 3.6)
...
def Action1(oWarrior, oLifeCycle):
    ...
    cl_action.CommonRemoveOwnerState(oWarrior, oLifeCycle, 33939, 0)
    cl_action.CommonRemoveOwnerState(oWarrior, oLifeCycle, 33940, 0)
    cl_action.CommonRemoveOwnerState(oWarrior, oLifeCycle, 33938, 0)
    cl_action.PassiveAddState(oWarrior, oLifeCycle, 33938, 0, {
...
'sArg': 'MainHoldCostMax' }))) and cl_evcon.CheckHasState(oWarrior, oEventCB, 33939):
            cl_action.CommonAddStateCount(oWarrior, oEventCB.GetCBLifeCycle(), 33938, (lambda *a: (Func717(*a, **{
'sArg': 'Remainder' }) + Func717(*a, **{
...
def DoCallBackAction5(oEventCB, oWarrior):
    if cl_evcon.CheckHasState(oWarrior, oEventCB, 33939) == 0 and cl_evcon.CheckFromMinorPerform(oWarrior, oEventCB):
        cl_action.CommonAddStateCount(oWarrior, oEventCB.GetCBLifeCycle(), 33938, 1, 0)


def DoCallBackAction10(oEventCB, oWarrior):
    if cl_evcon.EventCBCheckFromPointState(oWarrior, oEventCB, 33939) and cl_evcon.CheckTalent(oWarrior, oEventCB, 5432):
        cl_evact.PassiveCBSetPFArgs(oWarrior, oEventCB, 'MainHoldCostPF', 0)
        cl_evact.PassiveCBSetPFArgs(oWarrior, oEventCB, 'Remainder', 0)


def DoCallBackAction11(oEventCB, oWarrior):
    if cl_evcon.CheckHasState(oWarrior, oEventCB, 33939) == 0:
        cl_action.CommonAddStateCount(oWarrior, oEventCB.GetCBLifeCycle(), 33938, 1, 0)
