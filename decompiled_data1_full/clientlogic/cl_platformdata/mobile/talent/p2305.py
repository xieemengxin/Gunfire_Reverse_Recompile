# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/talent/p2305.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/talent/p2305.pyc
# Source Generated with Decompyle++
# File: p2305.pyc (Python 3.6)

import cl_msgcenter
import cl_action
import cl_condition
import cl_evact
import cl_evcon
from . import CTalent as CCustomPerform
from cl_commondefines import ATTACKERSUBMSG_NORMAL

def Action1(oWarrior, oLifeCycle):
    cl_action.CommonListenMsgCallBack(oWarrior, oLifeCycle, cl_msgcenter.MSG_WAR_KILL, ATTACKERSUBMSG_NORMAL, 0, 0, 0)


def Action2(oWarrior, oLifeCycle):
    cl_action.CommonListenMsgCallBack(oWarrior, oLifeCycle, cl_msgcenter.MSG_WAR_KILL, ATTACKERSUBMSG_NORMAL, 1, 0, 0)


def Action3(oWarrior, oLifeCycle):
    cl_action.CommonListenMsgCallBack(oWarrior, oLifeCycle, cl_msgcenter.MSG_WAR_KILL, ATTACKERSUBMSG_NORMAL, 2, 0, 0)
    cl_action.PassiveAddState(oWarrior, oLifeCycle, 32686, 0, { }, -1)


def DoCallBackAction0(oEventCB, oWarrior):
    cl_evact.EventCBSubCareerPerformColdTime(oWarrior, oEventCB, 50, 0)


def DoCallBackAction1(oEventCB, oWarrior):
    cl_evact.EventCBSubCareerPerformColdTime(oWarrior, oEventCB, 100, 0)


def DoCallBackAction2(oEventCB, oWarrior):
    if cl_evcon.CheckHasState(oWarrior, oEventCB, 32006):
        cl_action.CommonAddStateCount(oWarrior, oEventCB.GetCBLifeCycle(), 32686, 1, None)
    else:
        cl_evact.EventCBSubCareerPerformColdTime(oWarrior, oEventCB, 150, 0)


class CPerform(CCustomPerform):
    m_SID = 2305
    m_Name = '电流激荡'
    m_MaxLevel = 3
    m_MaxStack = 1
    m_ExtPerform = ()
    m_EnableActionInfo = {
        1: Action1,
        2: Action2,
        3: Action3 }
    m_DisableActionInfo = { }
    m_ColdDownCBActionInfo = { }
    m_CBFuncAction = {
        0: DoCallBackAction0,
        1: DoCallBackAction1,
        2: DoCallBackAction2 }
    m_BaseArgData = { }
    m_DieDisable = 0
    m_MaxUpgradeTimes = 2
    m_TalentType = 2
    m_IsRareTalent = 0
    m_Career = 104

