# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p39132.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p39132.pyc
# Source Generated with Decompyle++
# File: p39132.pyc (Python 3.6)
...
import cl_evact
import cl_evcon
from cl_perform.cartoon.defines import DirectPosCartoon, MonsterMoveCartoon, TimerCartoon
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgSelfPos(skill), (0, 0, 0), [
                cl_action.GetSkillCacheData(skill, SKILLCACHE_INT) * 1], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon6(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.SetMonsterAttackerPhase(skill, 4)
        cartoon = { }
...
class CCartoon5(MonsterMoveCartoon):
    m_SID = 5
    
    def Active(cls, skill):
        cl_action.AddClientEffect(skill, cl_action.GetPerformArgValue(skill, 'Effect', iDefault = 0), cl_action.CrtArgTargetGroundPos(skill), 0, (0, 0, 0))
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetLineByPos(skill, cl_action.CrtArgTargetGroundPos(skill)), speed = 0, iGround = 1)
...
class CCartoon4(MonsterMoveCartoon):
    m_SID = 4
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetSkillEndLine(skill), speed = 0, iGround = 1)
...
def Action(skill):
    cl_action.SetSkillCacheData(skill, SKILLCACHE_INT, cl_action.GetPerformArgValue(skill, 'Range', iDefault = 0))
    cl_action.SetMonsterAttackerPhase(skill, 6)
    cartoon = { }
