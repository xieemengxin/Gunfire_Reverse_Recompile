# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p39156.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p39156.pyc
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, OBJ_ALL, OBJ_ENEMY, SKILLCACHE_INT
...
class CCartoon7(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, i1, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 2, 0)), cl_action.GetMonsterMuzzlePos(skill, (0, 2, 0)), cl_action.StartAndEndAtSameHeight(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 2, 0)), cl_action.CrtArgDestPosDirPlane(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 2, 0)), cl_math.Vec3Normalize(cl_action.CrtArgSelfFace(skill)), 9999, i1 * 30)), 7, 200, 25, targettype = OBJ_ENEMY, effect = 0, liveTime = 0, trailEffect = 0, effectLiveTime = 2, radius = 1.5, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon1(DirectPosCartoon):
    ...
    def Hit(cls, skill):
        ...
        cl_action.PushHeroVictim(skill, cl_action.CrtArgSelfPos(skill), 30, 7, 10000, downSpeed = 0, fGravaty = 9.8, angle = 0)
...
class CCartoon13(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_math.Vec3Normalize(cl_action.CrtArgSelfFace(skill)), cl_math.Vec3Normalize(cl_action.CrtArgSelfFace(skill)), cl_math.Vec3Normalize(cl_action.CrtArgSelfFace(skill)), 0, 0, 0, targettype = OBJ_ALL, effect = None, liveTime = 0, trailEffect = None, effectLiveTime = 0, radius = 0, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon12(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 2, 0)), (0, 0, 0), cl_action.GetMonsterMuzzlePos(skill, (0, 2, 0)), 0, 0, 0, targettype = OBJ_ALL, effect = None, liveTime = 0, trailEffect = None, effectLiveTime = 0, radius = 0, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon2(SubductionCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetCartoonEnd(skill, 5), cl_action.GetSkillVarCache(skill, 'pos'), cl_action.GetSkillCacheData(skill, SKILLCACHE_INT))
...
class CCartoon8(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 55 - cl_action.GetPlayRound(skill) * 10), 1)
...
class CCartoon14(RayCastCartoon):
    ...
    def Active(cls, skill):
        cl_action.SetSkillVarCache(skill, 'pos', cl_action.GetCartoonStart(skill, 14))
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetSkillVarCache(skill, 'vicpos'), (0, 0, 0), cl_action.GetSkillVarCache(skill, 'vicpos'), 0, 0, 0, targettype = OBJ_ALL, effect = None, liveTime = 0, trailEffect = None, effectLiveTime = 0, radius = 0, flyoverdis = 0, passid = 0, extCheck = CRT_EXTCHECK_NONE)
