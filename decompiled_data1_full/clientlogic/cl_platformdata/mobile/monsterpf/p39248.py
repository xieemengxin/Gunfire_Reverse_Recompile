# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p39248.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p39248.pyc
# Source Generated with Decompyle++
# File: p39248.pyc (Python 3.6)
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, CRT_EXTCHECK_NONE, FIGHT3_KEY_IGNOREKNOCKBACK, OBJ_ALL, OBJ_FRIEND_NOSELF, SKILLCACHE_POS

class CCartoon15(DirectPosCartoon):
    m_SID = 15
...
class CCartoon22(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cartoon = { }
        CCartoon24.Init(skill, cartoon, casting = 0, index = 0)
...
class CCartoon4(TimerCartoon):
    ...
    def Active(cls, skill):
        cartoon = { }
        CCartoon22.Init(skill, cartoon, casting = 0, index = 0)
...
class CCartoon0(DirectPosCartoon):
    ...
    def Active(cls, skill):
        cartoon = { }
        CCartoon23.Init(skill, cartoon, casting = 0, index = 0)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgSelfPos(skill), cl_action.CrtArgSkillEndPos(skill), [
                60], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_FRIEND_NOSELF, pierceStatic = True, explosion = True, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon3(TimerCartoon):
    ...
    def Active(cls, skill):
        cl_action.AttackerAddState(skill, 8009, 2000, 1, { })
        cl_action.MonsterAttackerFacePos(skill, (1603, 500, 1236.92), 120)
        cartoon = { }
    ...
    def Trigger(cls, skill):
        cl_action.AttackerAddState(skill, 7985, 1000, 0, { })
        cartoon = { }
        CCartoon0.Init(skill, cartoon, casting = 1, index = 5)

    Trigger = classmethod(Trigger)
    
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, 343, 1)
...
class CCartoon2(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, (cl_math.CalDistance3D(cl_action.GetSkillVarCache(skill, 'start'), cl_action.GetSkillCacheData(skill, SKILLCACHE_POS)) / 50) * 100 + 4), 1)
...
class CCartoon5(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.AttackerUsePerform(skill, 39250, {
            'vPos': cl_action.CrtArgDestPosDirPlane(skill, cl_action.GetSkillVarCache(skill, 'start'), cl_math.Vec3Minus(cl_action.GetSkillCacheData(skill, SKILLCACHE_POS), cl_action.GetSkillVarCache(skill, 'start')), (cl_action.GetTimerCartoonCurTimes(skill, 5) - 1) * 2, 0),
...
class CCartoon1(TimerCartoon):
    ...
    def End(cls, skill):
        cartoon = { }
        CCartoon15.Init(skill, cartoon, casting = 0, index = 0)
...
class CCartoon21(MonsterDashCartoon):
    m_SID = 21
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (1613.09, 500, 1266.95), 0, 0, 9, True, False, False, checkDis = 0, start = (0, 0, 0), offSetY = 0, staticbreak = False, CheckTime = 0, StopDis = 0, CheckHalt = False)
...
def Action(skill):
    ...
    cl_action.MonsterAttackerFacePos(skill, (1613.09, 500, 1266.95), 360)
    if cl_math.CalDistance3D(cl_action.CrtArgSelfPos(skill), (1613.09, 500, 1266.95)) > 12:
        cl_action.SetSkillVarCache(skill, 'start', cl_action.CrtArgSelfPos(skill))
        cl_action.SetSkillVarCache(skill, 'end', (1613.09, 500, 1266.95))
