# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p21282.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p21282.pyc
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, FIGHT3_KEY_IGNOREKNOCKBACK, FIGHT3_KEY_IGNORETHUMP, OBJ_ENEMY
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 0, -4.3)), cl_action.CrtArgSkillEndPos(skill), [
                3.4], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False)
...
class CCartoon9(MonsterDashCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetSkillVarCache(skill, 'targetpos'), 45, 25, cl_action.ToInt(skill, (cl_action.CountDistance(skill, cl_action.CrtArgSelfPos(skill), cl_action.GetSkillVarCache(skill, 'targetpos')) + -3) / 0.45), True, True, True, checkDis = 0, start = (0, 0, 0), offSetY = 0, staticbreak = False, CheckTime = 0, StopDis = 0, CheckHalt = False)
