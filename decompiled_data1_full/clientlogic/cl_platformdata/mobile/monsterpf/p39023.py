# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p39023.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p39023.pyc
    ...
    def Hit(cls, skill):
        if cl_action.CheckVictimType(skill, WARRIOR_BUILD, OBJ_ALL):
            if cl_action.CheckVictimSID(skill, 1184):
                cl_action.ChangeVictimDefValue(skill, cl_action.ToInt(skill, cl_action.GetSkillVictimAttr(skill, 'HPMax') * -0.2), DAM_USE_HP)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetCartoonEnd(skill, 1), (0, 0, 0), [
                6], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon4(ThrowByPowerCartoon):
    ...
    def InitSuccess(cls, skill, i3, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 20, 0)), cl_action.CrtArgGetCustomDir(skill, cl_action.CrtArgMonsterMuzzlePos(skill, cartoon, (0, 20, 0)), cl_action.CrtArgTargetPos(skill, notContainDying = False), (0, 0, 0)), 80 if cl_action.GetMonsterPhase(skill) >= 3 else 65 if cl_action.GetMonsterPhase(skill) >= 2 else 50, 1, (0, 18, 0), (0.45, 0.45), 600, True, True, 0, 0, 0, 1, innerRadius = 2, pierce = 0, ignoreMonster = False, hitUnitBounciness = (0, 0), targettype = OBJ_ENEMY, hitovertype = HIT_OVER_NORMAL, changeRadius = 0, maxRadius = 5, ignorePetrochemical = False)
