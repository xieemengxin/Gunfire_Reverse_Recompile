# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p39241.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p39241.pyc
# Source Generated with Decompyle++
# File: p39241.pyc (Python 3.6)
...
import cl_evact
import cl_evcon
from cl_perform.cartoon.defines import DirectPosCartoon, MonsterDashCartoon, RayCastCartoon, TimerCartoon
from cl_commondefines import ATT_SHAPE_SECTOR, CRT_CHECK_SERVER, FIGHT3_KEY_IGNOREIMMOBILIZE, FIGHT3_KEY_IGNORETHUMP, OBJ_ENEMY, SKILLCACHE_SIGNSPEED
...
class CCartoon24(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.LockMonsterAttackerFace(skill)
        cl_action.SetSkillVarCache(skill, 'third', cl_action.CrtArgTargetPos(skill, notContainDying = True))
...
class CCartoon18(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.StartBackSwing(skill, cl_action.ToInt(skill, 130 / (cl_action.GetPlayRound(skill) * 0.25 + 0.75)))
...
class CCartoon7(RayCastCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 3.5, -5)), cl_action.GetMonsterMuzzlePos(skill, (0, 3.5, -5)), cl_action.StartAndEndAtSameHeight(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 3.5, -5)), cl_action.CrtArgDestPosDirPlane(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 3.5, -5)), cl_action.CrtArgGetCustomDir(skill, cl_action.GetMonsterMuzzlePos(skill, (0, 3.5, -5)), cl_action.GetSkillVarCache(skill, 'third'), (0, 0, 0)), 9999, (cl_action.GetSkillVarCache(skill, 'index') - 1) * 25)), 10, 100, 40, targettype = OBJ_ENEMY, effect = 0, liveTime = 0, trailEffect = 0, effectLiveTime = 0.2, radius = 2.5 if cl_action.CheckHasState(skill, 7997) else 3, flyoverdis = 0)
...
class CCartoon25(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.ToInt(skill, 60 / (cl_action.GetPlayRound(skill) * 0.25 + 0.75)), 1)
...
class CCartoon26(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cartoon = { }
        CCartoon25.Init(skill, cartoon, casting = 0, index = 0)
...
class CCartoon8(TimerCartoon):
    ...
    def Trigger(cls, skill):
        if cl_action.CheckHasState(skill, 7997):
...
class CCartoon5(DirectPosCartoon):
    ...
    def Hit(cls, skill):
        cl_action.PushHeroVictim(skill, cl_action.CrtArgSelfPos(skill), 8, 5, 10000, downSpeed = 3, fGravaty = 9.8, angle = 0)
        cl_action.WeaponDamage(skill, {
            'Att': 75 }, { }, sendPFMsg = False)
...
class CCartoon20(MonsterDashCartoon):
    m_SID = 20
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgTargetPos(skill, notContainDying = False), 26, 0, 8, True, False, False, checkDis = 5, start = (0, 0, 0), offSetY = 0, staticbreak = False, CheckTime = 0, StopDis = 0, CheckHalt = False)
...
class CCartoon3(MonsterDashCartoon):
    ...
    def End(cls, skill):
        cl_action.AttackerRemoveState(skill, 7116, bSameItem = False)
        cartoon = { }
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetPosByAttackerAndPointDistance(skill, 9), 0, 0, cl_math.CalDistance3D(cl_action.GetPosByAttackerAndPointDistance(skill, 9), cl_action.CrtArgSelfPos(skill)) / 0.4, True, False, False, checkDis = 0, start = cl_action.CrtArgSelfPos(skill), offSetY = 0, staticbreak = False, CheckTime = 0, StopDis = 0, CheckHalt = False)
...
class CCartoon1(TimerCartoon):
    ...
    def Trigger(cls, skill):
        if cl_action.CheckTargetAlive(skill, cl_action.GetSkillVID(skill)):
            cl_action.SetSkillCacheData(skill, SKILLCACHE_SIGNSPEED, cl_math.CalDistance3D(cl_action.CrtArgSelfPos(skill), cl_action.CrtArgTargetPos(skill, notContainDying = False)))
            cl_action.ServerSendSkillCache(skill, [
                SKILLCACHE_SIGNSPEED])
            if cl_action.GetSkillCacheData(skill, SKILLCACHE_SIGNSPEED) > 9:
                if cl_action.GetSkillCacheData(skill, SKILLCACHE_SIGNSPEED) > 16:
                    cl_action.TargetAddState(skill, 7116, 0, 0, { }, 0)
                    cartoon = { }
...
class CCartoon15(MonsterDashCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.CrtArgTargetPos(skill, notContainDying = False), 33, 0, 9, True, True, False, checkDis = 5, start = (0, 0, 0), offSetY = 0, staticbreak = False, CheckTime = 0, StopDis = 0, CheckHalt = False)
