# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p39096.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p39096.pyc
...
class CCartoon6(EntityCurveCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (0, 0, 0), cl_action.CrtArgTargetPos(skill, notContainDying = False), 0.8, 1033, 300, 18 if cl_action.GetPlayRound(skill) >= 3 else 15 if cl_action.GetPlayRound(skill) >= 2 else 12, 120 if cl_action.GetPlayRound(skill) >= 3 else 95 if cl_action.GetPlayRound(skill) >= 2 else 70, 0.3, targettype = OBJ_ENEMY, summonID = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[cl_action.GetTimerCartoonCurTimes(skill, 5) - 1], forceDel = True)
...
class CCartoon5(TimerCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, 16 if cl_action.GetPlayRound(skill) >= 3 else 20 if cl_action.GetPlayRound(skill) >= 2 else 24, cl_action.GetSkillCacheData(skill, SKILLCACHE_INT))
