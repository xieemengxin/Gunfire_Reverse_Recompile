# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/monsterpf/p39137.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/monsterpf/p39137.pyc
...
class CCartoon1(DirectPosCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetCartoonEnd(skill, 17), (0, 0, 0), [
                4], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE, multipleExplode = True, explosionDelay = 10)
...
class CCartoon6(TimerCartoon):
    ...
    def Trigger(cls, skill):
        cl_action.SetSkillVarCache(skill, 'summonpos', cl_action.GetSummonPosByBoxSplit(skill, 40 if cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) >= 3 else 30, 5 if cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) >= 3 else 4, 3, False, {
            cl_action.GetSkillCacheData(skill, SKILLCACHE_BALLISTICTYPE) + (cl_action.GetAllHeroCnt(skill, False, bUseRidingAloneCnt = True) - 1) * 1: 10 }, { }, True))
