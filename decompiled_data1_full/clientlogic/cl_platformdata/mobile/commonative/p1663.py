# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/commonative/p1663.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/commonative/p1663.pyc
# Source Generated with Decompyle++
# File: p1663.pyc (Python 3.6)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, cl_action.GetCartoonEnd(skill, 6), (0, 0, 0), [
                1.5], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = False, explosion = False, extCheck = CRT_EXTCHECK_NONE)
...
class CCartoon6(EntityCurveCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableCtrl(skill, (0, 0, 0), cl_action.CrtArgMissingPos(skill, 0), 0.3, 1037, 180, 10, 60, 0.5, targettype = OBJ_ENEMY, summonID = cl_action.GetSkillCacheData(skill, SKILLCACHE_LSTINT)[cl_action.GetTimerCartoonCurTimes(skill, 5) - 1], forceDel = True)
...
def Action(skill):
    ...
    cl_action.CreateRandomNumWarriorAtPointPos(skill, cl_action.CreateBoxPosList(skill, cl_math.Vec3Add(cl_action.CrtArgSelfPos(skill), (0, 4.2, 0)), ((cl_action.GetRoomPlayerCnt(skill) if cl_action.GetPlayCycle(skill) >= 9 else 1) * 0.6, 0.8, 0.8), cl_action.GetRoomPlayerCnt(skill) if cl_action.GetPlayCycle(skill) >= 9 else 1, 0.3, 0), WARRIOR_SUMMON, {
        1037: 10 }, {
        'Radius': 0.3 })
