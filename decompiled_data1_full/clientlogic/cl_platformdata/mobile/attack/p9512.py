# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/attack/p9512.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/attack/p9512.pyc
# Source Generated with Decompyle++
# File: p9512.pyc (Python 3.6)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            ...
            cls.EnableShow(skill, [
                skill.m_Cache['Radius']], attshape = ATT_SHAPE_SPHERE, targettype = OBJ_ENEMY, pierceStatic = True, explosion = True, multipleExplode = True, explosionDelay = 10)
...
class CCartoon0(ThrowByPowerCartoon):
    ...
    def Active(cls, skill):
        cl_action.SetSkillVarCache(skill, 'HitStatic', False)
    ...
    def Hit(cls, skill):
        ...
        if not cl_action.GetSkillVarCache(skill, 'HitStatic'):
            if cl_action.CheckHasInscription(skill, 4938):
                cartoon = { }
    ...
    def HitStatic(cls, skill):
        cl_action.SetSkillVarCache(skill, 'HitStatic', True)
        cartoon = { }
    ...
    def InitSuccess(cls, skill, i1, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.CrtArgMuzzlePos(skill, cartoon)):
                return None
            cls.EnableShow(skill, 0.2, (0, 16, 0), (0.99, 0.99), 300, True, False, 0, innerRadius = 0.1, pierce = 1, ignoreMonster = False, hitUnitBounciness = (0, 0), targettype = OBJ_ENEMY, hitovertype = HIT_OVER_NORMAL, changeRadius = 0, maxRadius = 3)
