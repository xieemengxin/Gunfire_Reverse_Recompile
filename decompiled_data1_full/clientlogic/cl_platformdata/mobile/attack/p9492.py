# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/attack/p9492.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/attack/p9492.pyc
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.CrtArgMuzzlePos(skill, cartoon)):
                return None
            cls.EnableShow(skill, 6, cl_action.GetWeaponAttackDis(skill), targettype = OBJ_ENEMY, UseLineRender = True, searchDis = cl_action.GetSkillVarCache(skill, 'searchDis'), searchNum = cl_action.GetSkillVarCache(skill, 'searchNum'), heightThreshold = 5)
...
class CCartoon1(TraceIntensifyLineCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.CrtArgMuzzlePos(skill, cartoon)):
                return None
            cls.EnableShow(skill, 2, cl_action.GetWeaponAttackDis(skill), targettype = OBJ_ENEMY, radius = 0.2, lockTeamer = False, needCheckBuff = False)
