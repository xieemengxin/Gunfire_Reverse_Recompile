# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/attack/p9213.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/attack/p9213.pyc
...
from cl_commondefines import ATT_SHAPE_SPHERE, CRT_CHECK_SERVER, MONSTER_PART_WEAKNESS, OBJ_ENEMY, SKILLCACHE_EXTRATRAJECTORY
...
class CCartoon1(ConvolutionCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetCartoonEnd(skill, 5)):
                return None
            cls.EnableShow(skill, 0.1, [
                10,
                15,
                20,
                25], cl_action.GetSkillAID(skill), skill.m_Cache['BulletSpeed'] * (3 if cl_action.CheckIsHover(skill, 5) else cl_math.CalDistance3D(cl_action.GetCartoonEnd(skill, 5), cl_action.CrtArgCameraCenterPos(skill, cartoon)) / 12), radius = 1, angle = 180, effect = 0, traileffect = 0, trailexisttime = 0.3)
    ...
    def InitSuccess(cls, skill, i1, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            ...
            cls.EnableShow(skill, 20, 18, [
                1.5], ATT_SHAPE_SPHERE, OBJ_ENEMY, cl_action.GetWeaponAttackDis(skill), skill.m_Cache['BulletSpeed'], 0, [
                None,
