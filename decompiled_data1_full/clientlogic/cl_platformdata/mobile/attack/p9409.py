# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/attack/p9409.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/attack/p9409.pyc
...
class CCartoon6(DirectPosCartoon):
    ...
    def Hit(cls, skill):
        ...
        cl_action.PushVictim(skill, cl_action.GetCartoonEnd(skill, 0), 20, 5, 10000, angle = 0, iCartoonSID = -1, iIgnoreStruckCD = 0, iClient = 0)
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.GetCartoonEnd(skill, 0), end = (0, 0, 0)):
                return None
...
class CCartoon0(DirectPosCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.EllipseRandomPoints(skill, cl_action.GetCartoonEnd(skill, 3), cl_action.GetMuzzlePos(skill), 10, cl_action.RandomFloat(cl_action.GetCartoonLoopID(skill, 1) / 2, cl_action.GetCartoonLoopID(skill, 1) / 1.7) if skill.m_Cache['Radius'] >= 4 else cl_action.RandomFloat(cl_action.GetCartoonLoopID(skill, 1) / 2, cl_action.GetCartoonLoopID(skill, 1) / 1.7) * (skill.m_Cache['Radius'] / 4), cl_action.RandomFloat(1, 3) if skill.m_Cache['Radius'] >= 4 else cl_action.RandomFloat(1, 3) * (skill.m_Cache['Radius'] / 4), cl_action.RandomFloat(10, 18), cl_action.RandomFloat(25, 40), cl_action.GetCartoonLoopID(skill, 1)), end = (0, 0, 0)):
                return None
            cls.EnableShow(skill, [
