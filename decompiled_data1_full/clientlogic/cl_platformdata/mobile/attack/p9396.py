# Path: /Users//个人空间/04 Gunfire/converted/converted_data1/clientlogic/cl_platformdata/mobile/attack/p9396.pyc
# RelativePath: clientlogic/cl_platformdata/mobile/attack/p9396.pyc
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            if not cls.EnableCheck(skill, start = cl_action.CrtArgCameraCenterPos(skill, cartoon)):
                return None
            cls.EnableShow(skill, skill.m_Cache['Pierce'], 35, targettype = OBJ_ENEMY, effect = 0, CanWheel = False, UseLineRender = True, issecondaryskill = True)
...
class CCartoon0(ChargeCartoon):
    ...
    def InitSuccess(cls, skill, index, cartoon):
        if not skill.m_CheckType == CRT_CHECK_SERVER:
            cls.EnableShow(skill, 32, 2, 0, True, True, effect = None, halfEnd = True, offsetTime = 0, breaktips = False)
