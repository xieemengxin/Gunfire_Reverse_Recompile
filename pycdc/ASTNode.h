...
public:
    enum Type {
        ...
        NODE_KW_NAMES_MAP, NODE_STARRED,
...
public:
    ...
    ASTClass(PycRef<ASTNode> code, PycRef<ASTNode> bases, PycRef<ASTNode> name,
             kwparam_t kwparams = kwparam_t())
        : ASTNode(NODE_CLASS), m_code(std::move(code)), m_bases(std::move(bases)),
          m_name(std::move(name)), m_kwparams(std::move(kwparams)) { }
...
class ASTStarred : public ASTNode {
public:
    ASTStarred(PycRef<ASTNode> value)
        : ASTNode(NODE_STARRED), m_value(std::move(value)) { }
...
public:
    ...
    void add_unpack(PycRef<ASTNode> value)
    {
        m_values.emplace_back(nullptr, std::move(value));
    }
...
public:
    ...
    ASTComprehension(PycRef<ASTNode> result, bool is_generator = false)
        : ASTNode(NODE_COMPREHENSION), m_result(std::move(result)),
          m_isGenerator(is_generator) { }
